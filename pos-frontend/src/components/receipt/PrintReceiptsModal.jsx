...
import { FaPrint, FaSpinner, FaCheck, FaTimes, FaReceipt, FaUtensils } from "react-icons/fa";
...
const PrintReceiptsModal = ({ 
  ...
  const getPrintStages = () => {
    ...
    switch (orderType) {
      case "DINE":
        ...
        return customerReceipts && customerReceipts.length > 0 ? ["kitchen", "customer"] : ["kitchen"];
      case "DELIVERY":
      case "PICKUP":
  ...
  const printReceipt = async (receiptData, receiptType) => {
    console.log(`Printing ${receiptType} receipt with data:`, receiptData.items);
    ...
    if (receiptData.deals && receiptData.deals.length > 0) {
      ...
      if (receiptData.deals[0].items && receiptData.deals[0].items.length > 0) {
        console.log("First deal item structure:", receiptData.deals[0].items[0]);
      }
    }
    ...
    const processedReceiptData = {
      ...
      items: receiptData.items?.map(item => {
        ...
        const basePrice = itemData.basePrice || itemData.originalPrice || 600;
        ...
        const taxRate = paymentMethod === 'CARD' ? parseFloat(taxRates.card || '0') : parseFloat(taxRates.cash || '');
      ...
      // Process deals data
      deals: receiptData.deals?.map(deal => {
    ...
    processedReceiptData.totalAmount = totalFinalAmount + totalDealAmount;
    ...
    return new Promise((resolve, reject) => {
      try {
        setPrintStatus(prev => ({
          ...prev,
          [receiptType]: { status: "printing", message: "Opening print dialog..." }
        }));
        ...
        const receiptTitle = isKitchen ? 
          (isNewlyAddedOnly ? 'KITCHEN ORDER - NEW ITEMS' : 'KITCHEN ORDER') : 
          'CUSTOMER RECEIPT';
        ...
        const printContent = `
          ...
          <body>
            <div class="receipt-header">
              ...
              <div>Order #${orderData._id ? orderData._id.slice(-6).toUpperCase() : 
                          orderData.orderId ? orderData.orderId.slice(-6).toUpperCase() : 
                          orderData.id ? orderData.id.slice(-6).toUpperCase() : 
                          Date.now().toString().slice(-6).toUpperCase()}</div>
            ...
            <div class="order-info">
              ...
              ${(orderData.customerInfo && (orderData.customerInfo.name || orderData.customerInfo.phone || orderData.customerInfo.address)) ? `
                <div class="customer-info">
                  ${isKitchen ? 
                    ...
                    `${orderData.customerInfo.name ? `<div><strong>Customer:</strong> ${orderData.customerInfo.name}</div>` : ''}
                     ...
                     ${orderData.customerInfo.address ? `<div><strong>Address:</strong> ${orderData.customerInfo.address}</div>` : ''}
                     ${orderData.customerInfo.table ? `<div><strong>Table:</strong> ${orderData.customerInfo.table}</div>` : ''}`
                  }
                </div>
              ` : ''}
            </div>
            
            ${isKitchen ? '<div class="priority-notice">⚡ PREPARE IMMEDIATELY ⚡</div>' : ''}
            
            ${isKitchen && isNewlyAddedOnly ? '<div class="priority-notice" style="background-color: #ff6b35;">🆕 NEWLY ADDED ITEMS ONLY 🆕</div>' : ''}
            
            <div class="items-section">
              <div style="font-weight: bold; margin-bottom: 8px; text-align: center;">
                ${isKitchen ? '🍽️ ITEMS TO PREPARE' : '📋 ITEMS ORDERED'}
              </div>
              
              ${processedReceiptData.items && processedReceiptData.items.length > 0 ? processedReceiptData.items.map((item, index) => {
                // Find menu group for this item
                console.log("item.totalTax", item);

                const menuGroup = processedReceiptData.menuGroups && processedReceiptData.menuGroups.find(menu => 
                  menu.menuId === item.menuId || menu.menuName === item.menuName
                );
                ...
                <div class="item-row">
                  <div class="item-with-logo">
                    ${menuLogo && !isKitchen ? `<img src="${menuLogo}" class="menu-logo" alt="${item.menuName} logo">` : ''}
                    <div>
                      <span class="item-name">${item.name}</span>
                      ${item.menuName && item.menuName !== 'General Items' ? `<span style="font-size: 9px; color: #666; font-style: italic; display: block;">(${item.menuName})</span>` : ''}
                    </div>
                  </div>
                  <span class="item-qty">x${item.quantity}</span>
                  ${!isKitchen ? `<span class="item-price">Rs${(item.price * item.quantity).toFixed(2)}</span>` : ''}
                </div>
                ${
                  !isKitchen ? `
                  
                  <div style="font-size: 9px; color: #666; margin-left: ${menuLogo ? '22px' : '10px'}; padding: 2px 0;">
                    <div>Base Price: Rs${item.basePrice.toFixed(2)}</div>
                    ${item.optionsPrice > 0 ? `<div>Options: Rs${item.optionsPrice.toFixed(2)}</div>` : ''}
                    ${item.itemDiscount > 0 ? `<div style="color: #28a745;">Discount: -Rs${item.itemDiscount.toFixed(2)}</div>` : ''}
                    <div style="color: #dc3545;">Tax (${item.itemTax}): Rs${(item.totalTax / item.quantity).toFixed(2)}</div>
                  </div>
                ` : ''}
                ${item.notes ? `<div style="font-size: 9px; color: #666; margin-left: ${menuLogo && !isKitchen ? '22px' : '10px'}; font-style: italic;">Note: ${item.notes}</div>` : ''}
                ${isKitchen && item.options && item.options.length > 0 ? `<div style="font-size: 9px; color: #666; margin-left: 10px;">Options: ${item.options.map(opt => typeof opt === 'string' ? opt : opt.name).join(', ')}</div>` : ''}
                ${!isKitchen && item.options && item.options.length > 0 ? `<div style="font-size: 9px; color: #666; margin-left: ${menuLogo && !isKitchen ? '22px' : '10px'};">Selected Options: ${item.options.map(opt => typeof opt === 'string' ? opt : `${opt.name} (+Rs${opt.price || 0})`).join(', ')}</div>` : ''}
              `;}).join('') : '<div>No items found</div>'}
            </div>
            
            ${processedReceiptData.deals && processedReceiptData.deals.length > 0 ? `
              <div class="deals-section" style="margin-top: 12px;">
                <div style="font-weight: bold; margin-bottom: 8px; text-align: center; color: #f6b100;">
                  🏷️ APPLIED DEALS
                </div>
                
                ${processedReceiptData.deals.map((deal, index) => {
                  return `
                  <div class="deal-row" style="border: 1px dashed #f6b100; padding: 6px; margin: 4px 0; border-radius: 3px; background: #fffbf0;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                      <span style="font-weight: bold; color: #f6b100;">🏷️ ${deal.name}</span>
                    ...
                    ${!isKitchen ? `
                      <div style="font-size: 9px; color: #666; margin: 2px 0;">
                        <div style="display: flex; justify-content: space-between;">
                          ...
                          <span>Rs${deal.dealPrice.toFixed(2)} each</span>
                        </div>
                        <div style="display: flex; justify-content: space-between;">
                          <span>Total Deal:</span>
                          <span style="font-weight: bold;">Rs${(deal.dealPrice * deal.quantity).toFixed(2)}</span>
                        </div>
                        <div style="display: flex; justify-content: space-between; color: #28a745;">
                          <span>You Save:</span>
                          <span style="font-weight: bold;">Rs${((deal.originalPrice - deal.dealPrice) * deal.quantity).toFixed(2)}</span>
                        </div>
                      </div>
                    ` : ''}
                    ...
                    ${deal.items && deal.items.length > 0 ? `
                      <div style="font-size: 8px; color: #666; margin-top: 4px; border-top: 1px dotted #ccc; padding-top: 3px;">
                        <strong>Includes:</strong> ${deal.items.map(item => {
                          ...
                          if (typeof item === 'string') {
                            ...
                            itemName = item.name || item.itemName || 
                                      (item.itemId && (typeof item.itemId === 'string' ? item.itemId : item.itemId.name)) || 
                                      'Unknown Item';
                            itemQuantity = item.quantity || 1;
                            selectedOptions = item.selectedOptions || [];
                          }
                          
                          let itemDisplay = itemName + ' (' + itemQuantity + 'x)';
                          ...
                          if (selectedOptions && selectedOptions.length > 0) {
                            const optionsText = selectedOptions.map(option => {
                              if (typeof option === 'string') {
                                ...
                                return option.name + (option.price > 0 ? ' (+Rs' + (option.price || 0) + ')' : '');
                              }
                              return 'Option';
                            }).join(', ');
                            ...
                            if (isKitchen) {
                              ...
                              itemDisplay += ' [Options: ' + optionsText + ']';
                            } else {
            ...
            ${!isKitchen && processedReceiptData.menuGroups && processedReceiptData.menuGroups.length > 1 ? `
              <div style="margin: 10px 0; padding: 8px; background: #f8f8f8; border-left: 3px solid #000;">
                ...
                ${processedReceiptData.menuGroups.map(menu =>
                  ...
                  <div style="display: flex; align-items: center; font-size: 9px; margin: 4px 0; padding: 3px; background: white; border-radius: 3px;">
                    ${menu.menuLogo ? `<img src="${menu.menuLogo}" style="width: 20px; height: 20px; margin-right: 6px; border-radius: 2px; object-fit: cover;" alt="${menu.menuName} logo">` : '<div style="width: 20px; height: 20px; margin-right: 6px; background: #ddd; border-radius: 2px; display: flex; align-items: center; justify-content: center; font-size: 8px;">🍽️</div>'}
                    <div style="flex: 1;">
                      <strong>${menu.menuName}:</strong> ${menu.itemCount} item(s) - Rs${menu.totalAmount.toFixed(2)}
                    </div>
                  </div>
                `).join('')}
              </div>
            ...
            ${isKitchen ? `
              <div style="text-align: center; font-weight: bold; font-size: 12px;">
                Total Items: ${processedReceiptData.items ? processedReceiptData.items.reduce((sum, item) => sum + item.quantity, 0) : 0}
              </div>
            ` : `
              <div class="total-section">
                <div class="info-row">
                  <span>Original Total:</span>
                  <span>Rs${processedReceiptData.originalAmount.toFixed(2)}</span>
                </div>
                ${processedReceiptData.itemDiscount > 0 ? `
                  <div class="info-row" style="color: #28a745;">
                    <span>Item Discounts:</span>
                    <span>-Rs${processedReceiptData.itemDiscount.toFixed(2)}</span>
                  </div>
                ` : ''}
                <div class="info-row">
                  <span>Subtotal:</span>
                ...
                ${processedReceiptData.voucherDiscount > 0 ? `
                  <div class="info-row" style="color: #10b981;">
                    ...
                    <span>-Rs${processedReceiptData.voucherDiscount.toFixed(2)}</span>
                  </div>
                ` : ''}
                ${processedReceiptData.totalTax > 0 ? `
                  <div class="info-row" style="color: #dc3545;">
                    <span>Tax (${orderData.paymentMethod || receiptData.paymentMethod || 'CASH'}):</span>
                ...
                ${processedReceiptData.totalDealSavings > 0 ? `
                  <div class="info-row" style="color: #8b5cf6;">
                    ...
                    <span>-Rs${processedReceiptData.totalDealSavings.toFixed(2)}</span>
                  </div>
                ` : ''}
                <div class="info-row" style="border-top: 1px solid #000; margin-top: 4px; padding-top: 4px; font-size: 14px;">
                  ...
                  <span><strong>Rs${processedReceiptData.totalAmount.toFixed(2)}</strong></span>
                </div>
                ${(processedReceiptData.itemDiscount + processedReceiptData.voucherDiscount + processedReceiptData.totalDealSavings) > 0 ? `
                  <div class="info-row" style="margin-top: 4px; color: #10b981;">
                    <span><strong>Total Savings:</strong></span>
                    <span><strong>Rs${(processedReceiptData.itemDiscount + processedReceiptData.voucherDiscount + processedReceiptData.totalDealSavings).toFixed(2)}</strong></span>
                  </div>
                ` : ''}
                ${orderData.paymentType || orderData.paymentMethod || receiptData.paymentMethod ? `
                  <div class="info-row" style="margin-top: 4px;">
                    <span>Payment Method:</span>
                    <span>${(orderData.paymentType || orderData.paymentMethod || receiptData.paymentMethod || 'CASH').toUpperCase()}</span>
                  </div>
                ` : ''}
                ${orderData.paymentStatus || receiptData.paymentStatus ? `
                  <div class="info-row" style="margin-top: 2px;">
                    <span>Payment Status:</span>
                    <span style="color: ${(orderData.paymentStatus || receiptData.paymentStatus) === 'PAID' ? '#28a745' : '#dc3545'};">${(orderData.paymentStatus || receiptData.paymentStatus || 'UNPAID').toUpperCase()}</span>
                  </div>
                ` : ''}
              </div>
            `}
            ...
            <div class="footer">
              ${isKitchen ? 
                'Kitchen Copy - Priority: HIGH<br>Prepare all items listed above' : 
                'Customer Copy<br>Thank you for your order!'
              }
              <br><br>
              Printed: ${new Date().toLocaleString()}
              <br><br>
              <div style="text-align: center; font-weight: bold; font-size: 10px; color: #666; border-top: 1px dashed #000; padding-top: 5px; margin-top: 5px;">
                Created By MiteMinds
              </div>
            </div>
          </body>
          </html>
        ...
        setTimeout(() => {
          ...
          const checkPrintStatus = () => {
            if (printWindow.closed) {
              setPrintStatus(prev => ({
                ...prev,
                [receiptType]: { status: "completed", message: "Printed successfully!" }
              }));
              resolve();
            } else {
  ...
  const handlePrintAll = async () => {
    ...
    try {
      ...
      if (printStages.includes("kitchen")) {
        ...
        if (kitchenReceipts && kitchenReceipts.length > 0) {
          kitchenData = {
            ...orderData,
            ...kitchenReceipts[0],
            items: orderData.items && orderData.items.length > 0 ? orderData.items : kitchenReceipts[0].items
          };
        }
      ...
      if (printStages.includes("customer") && (orderType === "DELIVERY" || orderType === "PICKUP" || (customerReceipts && customerReceipts.length > 0))) {
        ...
        if (customerReceipts && customerReceipts.length > 0) {
          customerData = {
            ...orderData,
            ...customerReceipts[0],
            items: orderData.items && orderData.items.length > 0 ? orderData.items : customerReceipts[0].items
          };
        }
      ...
      enqueueSnackbar("All receipts printed successfully!", { variant: "success" });
  ...
  const handlePrintIndividual = async (receiptType) => {
    ...
    try {
      ...
      enqueueSnackbar(`${receiptType === 'kitchen' ? 'Kitchen' : 'Customer'} receipt printed!`, { variant: "success" });
    } catch (error) {
      console.error(`Error printing ${receiptType} receipt:`, error);
      enqueueSnackbar(`Failed to print ${receiptType} receipt`, { variant: "error" });
    } finally {
      setIsLoading(false);
    }
  };
        ...
        >
          ...
          <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white p-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                {isNewlyAddedOnly ? <FaUtensils className="text-2xl" /> : <FaReceipt className="text-2xl" />}
                <div>
                  <h2 className="text-xl font-bold">
                    {isNewlyAddedOnly ? "Print New Kitchen Receipts" : "Print Receipts"}
                  </h2>
                  <p className="text-blue-100 text-sm">
                    Order #{orderData._id?.slice(-6) || 
                           orderData.orderId?.slice(-6) || 
                           orderData.id?.slice(-6) || 
                           Date.now().toString().slice(-6).toUpperCase()} - {orderType}
          ...
          <div className="p-6">
            ...
            <div className="space-y-4 mb-6">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h4 className="font-medium text-gray-800">
                      {isNewlyAddedOnly ? "New Items Kitchen Receipt" : "Kitchen Receipt"}
                    </h4>
                    <p className="text-sm text-gray-600">
                      {isNewlyAddedOnly ? "For newly added items preparation" : "For kitchen preparation"}
                    </p>
                  </div>
                </div>
              ...
              {!isNewlyAddedOnly && ((orderType === "DELIVERY" || orderType === "PICKUP") || (customerReceipts && customerReceipts.length > 0)) && (
                <div className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                  <div className="flex items-center space-x-3">
                    <FaReceipt className="text-green-500 text-xl" />
                    <div>
                      <h4 className="font-medium text-gray-800">Customer Receipt</h4>
                      <p className="text-sm text-gray-600">For customer records</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
              ...
              {isNewlyAddedOnly && (
                <div className="p-3 bg-orange-50 border border-orange-200 rounded-lg">
                  ...
                  <p className="text-xs text-orange-600 mt-1">
                    Kitchen staff will receive these receipts to prepare the additional items for this order.
                  </p>
                </div>
              )}
            </div>
            ...
            {printingStage === "completed" && (
              <div className="text-center py-4">
                ...
                <p className="text-green-600 font-medium">All receipts printed successfully!</p>
            ...
            <div className="flex items-center justify-between p-6 border-t border-gray-200 bg-gray-50">
              ...
              <div className="flex space-x-3">
                ...
                >
                  ...
                  <FaPrint />
                  <span>
                    {isNewlyAddedOnly ? "Print New Kitchen Receipt" : "Print All Receipts"}
                  </span>
                </button>
              </div>
            </div>
