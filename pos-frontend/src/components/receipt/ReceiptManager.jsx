          ...
          ${receipt.groupedItems ? 
            Object.values(receipt.groupedItems).map(group => `
              ${group.items.map(item => `
                ...
                ` : ''}
                ${item.notes ? `<div style="font-size: 8px; margin-left: 4px; color: #666;">Note: ${item.notes}</div>` : ''}
              `).join('')}
            `).join('')
            :
          ...
          <div class="items">
            ...
            ${receipt.groupedItems ? 
              ...
              receipt.items?.map(item => `
                ...
                ${item.notes ? `<div style="font-size: 10px; color: #666; margin-left: 10px;">Note: ${item.notes}</div>` : ''}
              `).join('') || ''
            }
          </div>
