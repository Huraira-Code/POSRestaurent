...
import { updateDeal, getAllItemsOfAdmin, getMyCreatedDeals } from '../../https/index';
...
const EditDealModal = ({ isOpen, onClose, onDealUpdated, deal }) => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'name' && value.trim()) {
      ...
      const isDuplicate = existingDeals.some(existingDeal => {
        const existingDealAdminId = existingDeal.adminId?._id || existingDeal.adminId;
        const currentDealAdminId = deal.adminId?._id || deal.adminId;
        return existingDeal._id !== deal._id && 
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      ...
      const isDuplicate = existingDeals.some(existingDeal => {
        ...
        return existingDeal._id !== deal._id && 
               existingDealAdminId === currentDealAdminId &&
               existingDeal.name.toLowerCase().trim() === formData.name.toLowerCase().trim();
      });
      if (isDuplicate) {
  ...
  const isItemsUnchanged = () => {
    ...
    return deal.items.every((originalItem, index) => {
      ...
      return currentItem && 
             (originalItem.itemId._id || originalItem.itemId) === currentItem.itemId &&
             originalItem.quantity === currentItem.quantity &&
             JSON.stringify(originalItem.selectedOptions || []) === JSON.stringify(currentItem.selectedOptions || []);
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        setErrors({ submit: response.data?.message || 'Failed to update deal' });
      }
    } catch (error) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1">Restaurant cannot be changed when editing</p>
                </div>
