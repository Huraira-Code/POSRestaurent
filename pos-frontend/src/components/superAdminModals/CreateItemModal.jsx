...
import { createItem, getCategoriesForAdmin, importItems, assignCategoriesToImportedItems, getAllItemsOfAdmin } from '../../https/index';
...
const CreateItemModal = ({ 
  ...
  const [isLoadingLocalCategories, setIsLoadingLocalCategories] = useState(false);
  ...
  const currentLoadingCategories = isLoadingCategories || isLoadingLocalCategories;
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (preselectedAdminId) {
        const filteredAdmins = allAdmins.filter(admin => admin._id !== preselectedAdminId);
  ...
  useEffect(() => {
    ...
    if (adminCategories.length === 0 && selectedAdmin && localAdminCategories.length === 0 && !isLoadingLocalCategories) {
      fetchCategoriesForAdmin(selectedAdmin._id);
    }
  }, [adminCategories, selectedAdmin, localAdminCategories.length, isLoadingLocalCategories]);
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      ...
      if (!validTypes.includes(file.type)) {
        setItemCreationError('Please select a valid image file (JPG, PNG, GIF, WebP)');
        return;
      }
  ...
  const handleAddOption = () => {
    ...
    const existingOption = itemFormData.options.find(opt => opt.name.toLowerCase() === name.trim().toLowerCase());
    if (existingOption) {
  ...
  const handleCreateItem = async () => {
    ...
    try {
      ...
      if (itemFormData.tax.cash && itemFormData.tax.card) {
        formData.append('tax', JSON.stringify({
      ...
      if (error.response?.data) {
        ...
        if (error.response.status === 400) {
          ...
          setItemCreationError('Access denied. You do not have permission to create items.');
        } else if (error.response.status === 404) {
          console.warn('Category not found');
          setItemCreationError('Selected category not found. Please select a valid category.');
        ...
        setItemCreationError('Network error. Please check your connection and try again.');
      } else {
  ...
  const handleImportItems = async () => {
    ...
    try {
      ...
      setImportSuccess(`Successfully imported ${items.length} items! Now assign categories to each item.`);
  ...
  const handleAssignCategories = async () => {
    ...
    try {
      ...
      if (error.response?.data) {
        setImportError(error.response.data.message || 'Failed to assign categories');
      } else {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-[#1a1a1a] p-8 rounded-lg w-[500px] max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <p className="text-[#a0a0a0] text-sm mb-6">
          Add menu items with pictures and assign to categories. You can create multiple items.
        </p>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <select 
              ...
              onChange={handleItemFormChange}
              className="w-full p-3 bg-[#262626] text-[#f5f5f5] rounded-lg border border-[#404040] focus:border-[#60a5fa] focus:outline-none"
              disabled={isCreatingItem || currentLoadingCategories || !itemFormData.adminId}
            >
              <option value="">
            ...
            {!itemFormData.adminId && (
              <p className="text-[#a0a0a0] text-xs mt-1">Please select an admin first to load categories</p>
            )}
          </div>
          ...
          {itemFormData.adminId && availableAdminsForImport.length > 0 && (
            <div className="border-t border-[#404040] pt-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-[#f5f5f5] text-sm font-medium">Import Items</h3>
                <button
                  type="button"
              ...
              {showImportSection && (
                <div className="space-y-3 p-3 bg-[#262626] rounded-lg border border-[#404040]">
                  <p className="text-[#a0a0a0] text-xs">
                    Import items from another admin. You'll need to assign categories to imported items.
                  </p>
                    ...
                    <div className="p-3 bg-green-900/20 border border-green-500 rounded text-green-300 text-sm">
                      ...
                      {importedItems.length > 0 && (
                        <div className="mt-2 pt-2 border-t border-green-500/30">
                          <p className="text-xs font-medium mb-1">Imported {importedItems.length} items:</p>
                          <div className="grid grid-cols-2 gap-1 max-h-20 overflow-y-auto">
                            {importedItems.slice(0, 6).map((item, index) => (
                              <div key={item._id} className="text-xs bg-green-800/30 px-2 py-1 rounded">
                                {item.name}
                              </div>
                            ))}
          ...
          {showCategoryAssignment && importedItems.length > 0 && (
            <div className="border-t border-[#404040] pt-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-[#f5f5f5] text-sm font-medium flex items-center gap-2">
                  <div className="w-6 h-6 bg-blue-500 rounded-full flex items-center justify-center">
                    <span className="text-white text-xs">{importedItems.length}</span>
                  </div>
              ...
              <p className="text-[#a0a0a0] text-xs mb-3">
                Assign categories to each imported item to complete the import process.
              </p>
              <div className="space-y-3 p-3 bg-[#262626] rounded-lg border border-[#404040] max-h-60 overflow-y-auto">
                {importedItems.map((item, index) => (
                  <div key={item._id} className="flex items-center gap-3 p-3 bg-[#1a1a1a] rounded border border-[#2a2a2a]">
                    ...
                    <div className="flex-1">
                      ...
                      {item.description && (
                        <p className="text-[#606060] text-xs mt-1 truncate">{item.description}</p>
                      )}
                    </div>
                    <div className="flex-1">
                      <select
                        value={categoryAssignments[item._id] || ''}
                        onChange={(e) => setCategoryAssignments(prev => ({
                          ...prev,
              ...
              <div className="mt-3 flex items-center justify-between">
                <div className="text-xs text-[#a0a0a0]">
                  {Object.values(categoryAssignments).filter(Boolean).length} of {importedItems.length} items have categories assigned
                </div>
                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${
                    Object.values(categoryAssignments).filter(Boolean).length === importedItems.length 
                      ? 'bg-green-500' 
                      : 'bg-yellow-500'
                  }`}></div>
                ...
                <button
                  ...
                  disabled={isAssigningCategories || Object.values(categoryAssignments).some(cat => !cat)}
                  className={`w-full py-3 px-3 rounded text-sm font-medium transition-colors mt-3 ${
                    isAssigningCategories || Object.values(categoryAssignments).some(cat => !cat)
                      ? 'bg-[#404040] text-[#606060] cursor-not-allowed'
                ...
                >
                  {isAssigningCategories ? (
                    ...
                    `Finalize Import (${Object.values(categoryAssignments).filter(Boolean).length}/${importedItems.length})`
                  )}
                </button>
            </div>
          )}
          ...
          <div>
            <label className="block text-[#f5f5f5] text-sm font-medium mb-2">Item Image *</label>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="block text-[#f5f5f5] text-sm font-medium">Item Options</label>
              <button
                type="button"
            ...
            {itemFormData.options.length > 0 && (
              <div className="mb-3 p-3 bg-[#262626] rounded-lg border border-[#404040]">
                <h4 className="text-[#f5f5f5] text-sm font-medium mb-2">Added Options ({itemFormData.options.length})</h4>
                <div className="space-y-2">
                  {itemFormData.options.map((option, index) => (
                    <div key={index} className="flex items-center justify-between p-2 bg-[#1a1a1a] rounded border border-[#404040]">
                      <div className="flex-1">
                        <span className="text-[#f5f5f5] font-medium">{option.name}</span>
                        <span className="text-[#10b981] ml-2">Rs{option.price}</span>
                      </div>
                      <button
                        type="button"
            ...
            {showAddOption && (
              <div className="p-4 bg-[#262626] rounded-lg border border-[#404040] mb-3">
                <h4 className="text-[#f5f5f5] text-sm font-medium mb-3">Add New Option</h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                  <div>
                    <label className="block text-[#f5f5f5] text-xs font-medium mb-1">Option Name *</label>
                    <input
                      type="text"
                      name="name"
                  ...
                  <div>
                    <label className="block text-[#f5f5f5] text-xs font-medium mb-1">Option Price *</label>
                    <input
                      type="number"
                      name="price"
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    disabled={isCreatingItem || !optionFormData.name || optionFormData.price === ''}
                    className={`px-4 py-2 rounded text-sm font-medium ${
                      isCreatingItem || !optionFormData.name || optionFormData.price === ''
                        ? 'bg-[#404040] text-[#606060] cursor-not-allowed'
                        : 'bg-[#10b981] hover:bg-[#059669] text-white'
                    }`}
                  >
            ...
            <p className="text-[#a0a0a0] text-xs">Add optional extras or variations for this item (e.g., Extra Cheese, Large Size)</p>
          </div>
          ...
          {onSuccess && (
            <div className="mt-4 p-4 bg-[#262626] rounded-lg">
              <h3 className="text-[#f5f5f5] text-sm font-medium mb-3">
                Items created in this session: Keep adding more items or click "Done" when finished.
              </h3>
        ...
        <div className="flex justify-between gap-3 mt-6">
          ...
          <button 
            ...
            disabled={isCreatingItem || !itemFormData.name || !itemFormData.price || !itemFormData.categoryId || !itemFormData.adminId || !itemFormData.tax.cash || !itemFormData.tax.card}
            className={`px-6 py-2 rounded-lg font-medium ${
              isCreatingItem || !itemFormData.name || !itemFormData.price || !itemFormData.categoryId || !itemFormData.adminId || !itemFormData.tax.cash || !itemFormData.tax.card
                ? 'bg-[#404040] text-[#606060] cursor-not-allowed'
