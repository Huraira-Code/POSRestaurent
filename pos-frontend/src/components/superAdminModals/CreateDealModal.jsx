...
import { createDeal, getAllAdmin, getAllItemsOfAdmin, getMyCreatedDeals } from '../../https/index';

const CreateDealModal = ({ isOpen, onClose, onDealCreated, preSelectedAdmin }) => {
  ...
  const fetchAdmins = async () => {
    try {
      ...
      if (response.data?.success) {
        ...
        // Auto-select the preSelectedAdmin if provided, otherwise first admin
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'name' && value.trim()) {
      ...
      // Check uniqueness only within the current admin's deals
      const isDuplicate = existingDeals.some(deal => {
        const dealAdminId = deal.adminId?._id || deal.adminId;
        return dealAdminId === currentAdminId &&
               deal.name.toLowerCase().trim() === value.toLowerCase().trim();
      });
      if (isDuplicate) {
        setErrors(prev => ({ 
          ...prev, 
          [name]: 'Deal name must be unique for this admin. A deal with this name already exists for this restaurant.' 
        }));
      } else if (errors[name]) {
  ...
  const toggleItemOption = (itemIndex, option) => {
    setFormData(prev => ({
      ...
      items: prev.items.map((item, i) => {
        if (i === itemIndex) {
          const isSelected = item.selectedOptions.some(opt => opt.name === option.name);
          if (isSelected) {
            return {
              ...item,
              selectedOptions: item.selectedOptions.filter(opt => opt.name !== option.name)
            };
          } else {
            return {
              ...item,
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      ...
      // Check if deal name is unique within this admin's deals
      const currentAdminId = formData.adminId || preSelectedAdmin?._id;
      const isDuplicate = existingDeals.some(deal => {
        const dealAdminId = deal.adminId?._id || deal.adminId;
        return dealAdminId === currentAdminId &&
      ...
      if (isDuplicate) {
        newErrors.name = 'Deal name must be unique for this admin. A deal with this name already exists for this restaurant.';
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        setErrors({ submit: response.data?.message || 'Failed to create deal' });
      }
    } catch (error) {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  {preSelectedAdmin ? (
                    <div>
                      ...
                      />
                      <p className="text-xs text-gray-500 mt-1">Admin is automatically selected based on current context</p>
                    </div>
                  ) : (
                    <select
                      name="adminId"
              ...
              <div className="space-y-4">
                <div>
                  ...
                  />
                  {errors.dealPrice && <p className="text-red-400 text-sm mt-1">{errors.dealPrice}</p>}
                </div>

                {/* Price Summary */}
                {originalPrice > 0 && formData.dealPrice && (
                  <div className="bg-[#262626] p-4 rounded-lg border border-[#404040]">
                    <h4 className="text-white font-medium mb-2">Price Summary</h4>
                    <div className="space-y-1 text-sm">
                      <div className="flex justify-between">
                        <span className="text-gray-300">Original Price:</span>
                        <span className="text-white">Rs {originalPrice.toFixed(2)}</span>
                      </div>
                      
                      {/* Original Price Breakdown */}
                      {(() => {
                        const hasOptions = formData.items.some(item => 
                          item.selectedOptions && item.selectedOptions.length > 0
                        );
                        
                        if (hasOptions) {
                          const baseItemsTotal = formData.items.reduce((total, dealItem) => {
                            const item = availableItems.find(item => item._id === dealItem.itemId);
                            return item ? total + (item.price * dealItem.quantity) : total;
                          }, 0);
                          
                          const optionsTotal = formData.items.reduce((total, dealItem) => {
                            return total + dealItem.selectedOptions.reduce((optTotal, option) => {
                              return optTotal + (option.price * dealItem.quantity);
                            }, 0);
                          }, 0);
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-white">Rs {parseFloat(formData.dealPrice).toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between border-t border-[#404040] pt-1">
                        ...
                        <span className={savings > 0 ? "text-green-400" : "text-red-400"}>
                          Rs {Math.abs(savings).toFixed(2)} {savings > 0 ? "saved" : "extra"}
                        </span>
                      </div>
                      {savings > 0 && (
                        <div className="flex justify-between">
            ...
            <div>
              ...
              {!formData.adminId && !preSelectedAdmin && (
                <p className="text-gray-400 text-sm mb-4">Please select an admin first to add items.</p>
              )}
              ...
              <div className="space-y-3">
                {formData.items.map((dealItem, index) => (
                  <div key={index} className="bg-[#262626] p-4 rounded-lg border border-[#404040]">
                    <div className="flex items-center gap-4">
                      <div className="flex-1">
                        <select
                          value={dealItem.itemId}
                          onChange={(e) => updateDealItem(index, 'itemId', e.target.value)}
                        ...
                        {errors[`item_${index}_itemId`] && (
                          <p className="text-red-400 text-sm mt-1">{errors[`item_${index}_itemId`]}</p>
                        )}
                      </div>
                    ...
                    {dealItem.itemId && (
                      <div className="mt-3">
                        {(() => {
                          ...
                          const basePrice = item.price * dealItem.quantity;
                          const optionsPrice = dealItem.selectedOptions.reduce((total, option) => {
                          ...
                          return (
                            <div className="space-y-3">
                              <div className="text-sm text-gray-400">
                                ...
                                {optionsPrice > 0 && (
                                  <div>Options: Rs {optionsPrice.toFixed(2)}</div>
                                )}
                                <div className="font-medium">Subtotal: Rs {(basePrice + optionsPrice).toFixed(2)}</div>
                              </div>
                              
                              {/* Selected Options Display */}
                              {dealItem.selectedOptions && dealItem.selectedOptions.length > 0 && (
                                <div className="border-t border-[#404040] pt-3">
                                  <h5 className="text-sm font-medium text-green-400 mb-2">Selected Options:</h5>
                                  <div className="flex flex-wrap gap-2">
                                    {dealItem.selectedOptions.map((option, optIndex) => (
                                      <div
                                        key={optIndex}
                                        className="flex items-center space-x-2 px-3 py-1 bg-green-900/30 border border-green-500/50 rounded-full"
                                      >
                                        <span className="text-sm text-green-300">{option.name}</span>
                                        <span className="text-xs text-green-400">+Rs{option.price}</span>
                                        <button
                                          type="button"
                                          onClick={() => toggleItemOption(index, option)}
                              ...
                              {item.options && item.options.length > 0 && (
                                <div className="border-t border-[#404040] pt-3">
                                  <h5 className="text-sm font-medium text-gray-300 mb-2">Available Options:</h5>
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                    {item.options.map((option, optIndex) => (
                                      ...
                                      >
                                        <input
                                          type="checkbox"
                                          checked={dealItem.selectedOptions.some(opt => opt.name === option.name)}
                                          onChange={() => toggleItemOption(index, option)}
                                          className="text-[#60a5fa] focus:ring-[#60a5fa] rounded"
                                        />
                                        <span className="text-sm text-white flex-1">{option.name}</span>
                                        <span className="text-sm text-gray-400">+Rs{option.price}</span>
                                      </label>
                                    ))}
                                  </div>
                                </div>
              ...
              {formData.items.length === 0 && (formData.adminId || preSelectedAdmin) && !isLoadingItems && (
                <p className="text-gray-400 text-center py-8">No items added yet. Click "Add Item" to get started.</p>
              )}
            </div>
