...
import { updateOrder, getItems, getPayment, generateCustomerReceipts, completeOrder, validateVoucher as validateVoucherAPI } from "../../https/index";
import PrintReceiptsModal from "../receipt/PrintReceiptsModal";

const EditOrderModal = ({ order, onClose, onOrderUpdated, onCompleteOrder }) => {
  ...
  const [showNewKitchenReceiptsModal, setShowNewKitchenReceiptsModal] = useState(false);
  ...
  const [showCompleteOrderSection, setShowCompleteOrderSection] = useState(false);
  ...
  const [isPrintingCustomerReceipts, setIsPrintingCustomerReceipts] = useState(false);
  ...
  useEffect(() => {
    console.log("🍽️ newKitchenReceipts changed:", newKitchenReceipts.length, newKitchenReceipts);
  }, [newKitchenReceipts]);

  // Debug: Track newly added items and button visibility
  useEffect(() => {
    const newlyAddedCount = orderItems.filter(item => item.isNewlyAdded).length;
    const hasNewItems = orderItems.some(item => item.isNewlyAdded);
    const showButton = newKitchenReceipts.length > 0 || (hasNewItems && hasBeenUpdated);
    console.log("🆕 Newly added items:", newlyAddedCount, "hasBeenUpdated:", hasBeenUpdated, "shouldShowButton:", showButton);
  ...
  const completeDeliveryPickupMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      enqueueSnackbar("Order completed and marked as paid!", { variant: "success" });
      onOrderUpdated();
  ...
  const completeOrderMutation = useMutation({
    mutationFn: async ({ orderId, paymentDetails }) => {
      ...
      try {
        ...
        return {
          ...
          data: {
            ...response.data,
            customerReceipts: receiptData.data?.customerReceipts || receiptData.customerReceipts || []
          }
        };
      } catch (receiptError) {
        console.warn('Error generating receipts, but order was completed:', receiptError);
        return response;
      }
    },
    onMutate: () => {
    ...
    onSuccess: (response) => {
      ...
      const customerReceipts = response?.data?.customerReceipts || data?.customerReceipts || [];
      if (customerReceipts && customerReceipts.length > 0) {
        console.log("Generated customer receipts with detailed pricing:", customerReceipts);
        setReceiptData({
          ...order,
  ...
  const updateOrderMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      if (data && data.newKitchenReceipts && data.newKitchenReceipts.length > 0) {
        console.log("🍽️ New kitchen receipts for added items:", data.newKitchenReceipts);
        console.log("🍽️ Auto-print enabled:", autoPrintNewItems);
        console.log("🍽️ Setting newKitchenReceipts state to:", data.newKitchenReceipts);
        ...
        if (autoPrintNewItems) {
          ...
          enqueueSnackbar(`${data.newKitchenReceipts.length} new kitchen receipt(s) ready to print!`, { 
            variant: "info",
            action: (
              <button 
    ...
    onError: (error) => {
      setIsUpdatingOrder(false);
      enqueueSnackbar(error.response?.data?.message || "Failed to update order", { 
        variant: "error" 
      });
    },
  });
  ...
  const generateCustomerReceiptsMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      enqueueSnackbar("Customer receipts generated successfully!", { variant: "success" });
    ...
    onError: (error) => {
      setIsPrintingCustomerReceipts(false);
      enqueueSnackbar(error.response?.data?.message || "Failed to generate customer receipts", { 
        variant: "error" 
      });
    },
  });
  ...
  const paymentMutation = useMutation({
    ...
    onError: (error) => {
      enqueueSnackbar(error.response?.data?.message || "Failed to process payment", { 
        variant: "error" 
      });
    },
  });
  ...
  const addItemToOrder = (item, options = []) => {
    ...
    const cartItem = {
      ...
      // Names and display
      name: options.length > 0 ? `${item.name} (${options.map(opt => opt.name).join(', ')})` : item.name,
      ...
      itemTotalAfterDiscount: ((item.price + optionsTotal) - (item.discount || 0)) * 1, // After item discount
  ...
  const handleUpdateOrder = () => {
    if (orderItems.length === 0 && (!safeOrder.deals || safeOrder.deals.length === 0)) {
      enqueueSnackbar("Order must have at least one item or deal", { variant: "error" });
      return;
    }
  ...
  const getOptionsTotal = (item) => {
    ...
    return item.options.reduce((total, option) => total + (option.price || 0), 0);
  };
  ...
  const getItemBasePrice = (item) => {
    ...
    return item.originalPrice || item.totalPrice || item.basePrice || 0;
  };
  ...
  const calculateOriginalTotal = () => {
    ...
    return itemsOriginalTotal + dealsOriginalTotal;
  };
  ...
  const calculateTax = (subtotal, paymentMethod) => {
    ...
    return orderItems.reduce((totalTax, item) => {
      try {
        ...
        let taxRate = 0;
        if (item.tax) {
          if (paymentMethod.toLowerCase() === 'card' || paymentMethod.toLowerCase() === 'online') {
  ...
  const validateVoucher = async (code) => {
    ...
    try {
      ...
      const menuIds = [...new Set(orderItems.map(item => item.menuId).filter(Boolean))];
      ...
      if (response.data.success && response.data.voucher) {
        ...
        enqueueSnackbar(`Voucher applied! Discount: Rs${response.data.discount}`, { variant: "success" });
      } else {
        setVoucherDiscount(0);
        setAppliedVoucher(null);
        enqueueSnackbar(response.data.message || "Invalid voucher code", { variant: "error" });
      }
    } catch (error) {
      console.error("Voucher validation error:", error);
      setVoucherDiscount(0);
      setAppliedVoucher(null);
      const errorMessage = error.response?.data?.message || "Error validating voucher";
  ...
  const handleCompleteOrder = (paymentType) => {
    try {
      ...
      enqueueSnackbar('Error completing order: ' + error.message, { variant: "error" });
    }
  };
  ...
  const isAnyOperationInProgress = isCompletingOrder || isPrintingUnpaidReceipt || isPrintingCustomerReceipts || isUpdatingOrder;
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-[#1a1a1a] rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden">
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
          ...
          <div className="mb-6 p-4 bg-[#262626] rounded-lg">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                ...
                <span className="ml-2 text-[#f5f5f5] font-medium">{safeOrder.orderType}</span>
              </div>
              <div>
                <span className="text-[#a0a0a0]">Status:</span>
                <span className="ml-2 text-[#f5f5f5] font-medium">{safeOrder.orderStatus}</span>
              </div>
              <div>
                <span className="text-[#a0a0a0]">Payment Status:</span>
                <div className="flex items-center space-x-2">
                  <span className={`font-medium ${
                    safeOrder.paymentStatus === 'PAID' ? 'text-[#10b981]' : 
                    safeOrder.paymentStatus === 'UNPAID' ? 'text-[#ef4444]' : 'text-[#f5f5f5]'
                  }`}>
              ...
              <div>
                ...
                <span className="ml-2 text-[#f5f5f5] font-medium">{safeOrder.paymentMethod}</span>
              </div>
              <div>
                <span className="text-[#a0a0a0]">Created:</span>
                <span className="ml-2 text-[#f5f5f5] font-medium">
                  {new Date(safeOrder.printedAt || safeOrder.createdAt).toLocaleString()}
                </span>
              </div>
              <div>
              ...
              <div>
                <span className="text-[#a0a0a0]">Total Items:</span>
                <span className="ml-2 text-[#f5f5f5] font-medium">
            ...
            {safeOrder.customerInfo?.name && (
              <div className="mt-3 p-2 bg-[#3b82f6]/20 border border-[#3b82f6]/50 rounded-lg">
                <div className="flex justify-between items-center">
                  <span className="text-[#3b82f6] text-sm font-medium">Customer:</span>
                  <span className="text-[#3b82f6] font-medium">{safeOrder.customerInfo.name}</span>
                </div>
            ...
            {safeOrder.voucherCode && (
              <div className="mt-3 p-2 bg-[#10b981]/20 border border-[#10b981]/50 rounded-lg">
                <div className="flex justify-between items-center">
                  <span className="text-[#10b981] text-sm font-medium">Voucher Applied:</span>
                  <span className="text-[#10b981] font-medium">{safeOrder.voucherCode}</span>
                </div>
          ...
          {isOrderCompleted && (
            <div className="mb-4 p-3 bg-[#f59e0b]/20 border border-[#f59e0b]/50 rounded-lg">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <div className="text-[#f59e0b] font-medium">Order Completed</div>
                  <div className="text-[#f59e0b] text-sm">
                    This order is completed. You cannot add or remove items, but you can print receipts and change payment status.
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-[#f5f5f5]">Order Items</h3>
              <div className="flex items-center space-x-3">
                ...
                {!(order.orderType === "DELIVERY" || order.orderType === "PICKUP") && (
                  <label className={`flex items-center space-x-2 text-sm ${canEditOrder ? 'text-[#a0a0a0]' : 'text-[#606060]'}`}>
                    <input
                      type="checkbox"
                      checked={autoPrintNewItems}
                      onChange={(e) => setAutoPrintNewItems(e.target.checked)}
                ...
                {canEditOrder && !(order.orderType === "DELIVERY" || order.orderType === "PICKUP") && (
            ...
            <div className="space-y-3">
              {orderItems.map((item) => (
                <div key={item._id || item.itemId} className={`flex items-center justify-between p-4 rounded-lg transition-colors ${
                  item.isNewlyAdded 
                  ...
                  <div className="flex-1">
                    <div className="flex items-center space-x-2 mb-2">
                      <h4 className="text-[#f5f5f5] font-medium">{item.name || item.originalName}</h4>
                      {item.isNewlyAdded && (
                    ...
                    <div className="flex items-center space-x-3 mb-2">
                      ...
                      {item.categoryName && item.categoryName !== 'General' && (
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center space-x-2">
                        <span className="text-[#a0a0a0] text-xs">Base:</span>
                        <span className="text-[#a0a0a0] text-sm">Rs{getItemBasePrice(item).toFixed(2)}</span>
                        
                        {getOptionsTotal(item) > 0 && (
                          <>
                            <span className="text-[#f6b100] text-xs">+ Options:</span>
                            <span className="text-[#f6b100] text-sm">Rs{getOptionsTotal(item).toFixed(2)}</span>
                          </>
                        )}
                        
                        {getItemDiscount(item) > 0 && (
                          <>
                            <span className="text-[#ef4444] text-xs">- Discount:</span>
                            <span className="text-[#ef4444] text-sm">Rs{getItemDiscount(item).toFixed(2)}</span>
                          </>
                        )}
                      </div>
                      ...
                      {/* Tax info (simplified) */}
                      {item.tax && (item.tax.cash !== '0' || item.tax.card !== '0') && (
                        <div className="text-[#3b82f6] text-xs">
                          💰 Tax: Cash {item.tax.cash}% | Card {item.tax.card}%
                        </div>
                      )}
                    </div>
                    ...
                    {item.options && item.options.length > 0 && (
                      <div className="mt-2">
                        <p className="text-[#a0a0a0] text-xs">Available Options:</p>
                        {item.options.slice(0, 2).map((option, index) => (
                          <p key={index} className="text-[#f6b100] text-xs ml-2">
                            • {option.name} (+Rs{option.price || 0})
                          </p>
                        ))}
                        {item.options.length > 2 && (
                          <p className="text-[#606060] text-xs ml-2">
                    ...
                    {item.selectedOptions && item.selectedOptions.length > 0 && (
                      <div className="mt-2">
                        <p className="text-[#a0a0a0] text-xs">Selected Options:</p>
                        {item.selectedOptions.map((option, index) => (
                          <p key={index} className="text-[#10b981] text-xs ml-2">
                            ✓ {option.name} (+Rs{option.price || 0})
                          </p>
                        ))}
                      </div>
                  ...
                  <div className="flex flex-col items-end space-y-2 ml-4">
                    {/* Quantity */}
                    <div className="flex items-center space-x-2">
                      <span className="text-[#f5f5f5] font-medium text-sm">Qty: {item.quantity}</span>
                    </div>
                    
                    {/* Simplified Total Price */}
                    <div className="text-right">
                      <div className="text-[#f5f5f5] font-bold text-lg">
                        Rs{(getItemFinalPrice(item) * item.quantity).toFixed(2)}
                      </div>
                      {getItemDiscount(item) > 0 && (
                        <div className="text-[#ef4444] text-xs">
                          Save Rs{(getItemDiscount(item) * item.quantity).toFixed(2)}
                        </div>
                      )}
                    </div>
            ...
            {orderItems.length === 0 && (!safeOrder.deals || safeOrder.deals.length === 0) && (
              <div className="text-center py-8 text-[#606060]">
                No items or deals in order. Add some items or deals to continue.
              </div>
            )}
          </div>
          ...
          {safeOrder.deals && safeOrder.deals.length > 0 && (
            <div className="mb-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-[#f5f5f5]">Applied Deals</h3>
                <span className="px-3 py-1 bg-[#f6b100]/20 text-[#f6b100] text-sm rounded-full">
                  {(safeOrder.deals || []).length} deal{(safeOrder.deals || []).length > 1 ? 's' : ''}
                </span>
              </div>
              ...
              <div className="space-y-3">
                {(safeOrder.deals || []).map((deal, index) => {
                  ...
                  if (deal.items && deal.items.length > 0) {
                    ...
                    console.log('First deal item selectedOptions:', deal.items[0].selectedOptions);
                  }
                  console.log(`=== END DEAL ${index} DEBUG ===`);
                  
                  return (
                    <div key={deal.dealId || index} className="bg-[#f6b100]/10 border border-[#f6b100]/30 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-green-400 text-xs">
                          Save Rs{((deal.originalPrice - deal.dealPrice) * deal.quantity).toFixed(2)}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between text-sm">
                      <div className="flex items-center space-x-4">
                        <span className="text-[#a0a0a0]">Quantity: <span className="text-[#f5f5f5] font-medium">{deal.quantity}</span></span>
                        <span className="text-[#a0a0a0]">
                          Original: <span className="line-through text-[#ef4444]">Rs{(deal.originalPrice * deal.quantity).toFixed(2)}</span>
                        </span>
                      </div>
                    </div>
                    ...
                    {deal.items && deal.items.length > 0 && (
                      <div className="mt-3 pt-3 border-t border-[#f6b100]/20">
                        <p className="text-[#a0a0a0] text-xs mb-2">Included Items:</p>
                        <div className="space-y-2">
                          {deal.items.map((item, itemIndex) => (
                            <div key={itemIndex} className="bg-[#262626] text-[#f5f5f5] text-xs p-2 rounded">
                              <div className="flex items-center justify-between">
                                <span className="font-medium">{item.name}</span>
                                <span className="text-[#a0a0a0]">({item.quantity}x)</span>
                              </div>
                              
                              {/* Show selected options for deal items */}
                              {item.selectedOptions && item.selectedOptions.length > 0 && (
                                <div className="mt-1 pl-2 border-l-2 border-[#f6b100]/30">
                                  <p className="text-[#a0a0a0] text-xs mb-1">Selected Options:</p>
                                  <div className="flex flex-wrap gap-1">
                                    {item.selectedOptions.map((option, optionIndex) => (
                                      <span key={optionIndex} className="bg-[#f6b100]/20 text-[#f6b100] text-xs px-1 py-0.5 rounded">
                                        {typeof option === 'string' ? option : option.name}
                                        {option.price && option.price > 0 && ` (+Rs${option.price})`}
                                      </span>
                                    ))}
                                  </div>
                                </div>
              ...
              <div className="mt-4 p-3 bg-[#262626] rounded-lg">
                <div className="flex justify-between items-center">
                  <span className="text-[#a0a0a0] font-medium">Total Deals Value:</span>
                  <span className="text-[#f6b100] font-bold">
                    Rs{(safeOrder.deals || []).reduce((sum, deal) => sum + (deal.dealPrice * deal.quantity), 0).toFixed(2)}
                  </span>
                </div>
                <div className="flex justify-between items-center mt-1">
                  <span className="text-[#a0a0a0] font-medium">Total Savings:</span>
                  <span className="text-green-400 font-bold">
                    Rs{(safeOrder.deals || []).reduce((sum, deal) => sum + ((deal.originalPrice - deal.dealPrice) * deal.quantity), 0).toFixed(2)}
                  </span>
                </div>
              </div>
            </div>
          ...
          {showAddItems && canEditOrder && !(order.orderType === "DELIVERY" || order.orderType === "PICKUP") && (
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-[#f5f5f5] mb-4">Available Items</h3>
              <div className="grid grid-cols-2 gap-3 max-h-60 overflow-y-auto">
                {availableItems.map((item) => (
                  ...
                  >
                    ...
                    {item.discount && item.discount > 0 ? (
                      <div className="flex items-center space-x-2">
                        <span className="text-[#a0a0a0] text-sm line-through">Rs{item.price}</span>
                        <span className="text-[#10b981] text-sm font-medium">
                          Rs{(item.price - item.discount).toFixed(2)}
                        </span>
                      </div>
                    ) : (
                      <p className="text-[#a0a0a0] text-sm">Rs{item.price}</p>
                    )}
          ...
          <div className="mb-6 space-y-3">
            <h3 className="text-lg font-semibold text-[#f5f5f5] mb-4">Order Summary</h3>
            ...
            <div className="p-4 bg-[#262626] rounded-lg">
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[#a0a0a0] font-medium">Rs{calculateOriginalTotal().toFixed(2)}</span>
                </div>
                ...
                {calculateTotalDiscount() > 0 && (
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-[#ef4444] font-medium">-Rs{calculateTotalDiscount().toFixed(2)}</span>
                  </div>
                )}
                ...
                {voucherDiscount > 0 && (
                  <div className="flex justify-between items-center">
                    <span className="text-[#10b981]">Voucher Discount:</span>
                    <span className="text-[#10b981] font-medium">-Rs{voucherDiscount.toFixed(2)}</span>
                  </div>
                )}
                ...
                {calculateDealSavings() > 0 && (
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-[#8b5cf6] font-medium">-Rs{calculateDealSavings().toFixed(2)}</span>
                  </div>
                )}
                
                {/* Final subtotal */}
                <div className="border-t border-[#404040] pt-3 flex justify-between items-center">
                  <span className="text-[#f5f5f5] font-medium text-lg">Subtotal:</span>
                  <span className="text-[#f5f5f5] font-bold text-xl">Rs{calculateTotal().toFixed(2)}</span>
                </div>
                
                {/* Total savings summary */}
                {calculateTotalSavings() > 0 && (
                  <div className="flex justify-between items-center text-sm bg-[#10b981]/20 -mx-4 -mb-4 mt-3 p-3 rounded-b-lg">
                    <span className="text-[#10b981] font-medium">🎉 You Save:</span>
                    <span className="text-[#10b981] font-bold">Rs{calculateTotalSavings().toFixed(2)}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            {showCompleteOrderSection && (
              <div className="mt-6 p-6 bg-[#262626] rounded-lg border-t border-[#404040]">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-bold text-[#f5f5f5]">Complete Order</h3>
                  <button
                    onClick={() => {
                      setShowCompleteOrderSection(false);
                      setVoucherCode("");
                ...
                <div className="mb-6 p-4 bg-[#1a1a1a] rounded-lg">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-[#a0a0a0]">Original Total:</span>
                      <span className="text-[#a0a0a0]">Rs{calculateOriginalTotal().toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-[#a0a0a0]">Item Discounts:</span>
                      <span className="text-[#ef4444]">-Rs{calculateTotalDiscount().toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-[#a0a0a0]">Subtotal:</span>
                      <span className="text-[#f5f5f5]">Rs{calculateTotal().toFixed(2)}</span>
                    </div>
                    {voucherDiscount > 0 && (
                      <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-[#f5f5f5]">Rs{calculateTax(calculateTotal(), paymentMethod).toFixed(2)}</span>
                    </div>
                    <div className="border-t border-[#404040] pt-2 flex justify-between font-bold">
                      ...
                      <span className="text-[#f6b100]">
                        Rs{(calculateTotal() + calculateTax(calculateTotal(), paymentMethod) - voucherDiscount).toFixed(2)}
                      </span>
                    </div>
                    {calculateTotalSavings() > 0 && (
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <span className="text-[#f5f5f5] font-medium">{method.label}</span>
                          <div className="text-xs text-[#a0a0a0]">
                            Tax: Rs{calculateTax(calculateTotal(), method.value).toFixed(2)}
                          </div>
                        </div>
                      </label>
                    ))}
                ...
                <div className="flex justify-end space-x-3">
                  ...
                  >
                    {isCompletingOrder ? (
                      ...
                      `Complete Order - Rs${(calculateTotal() + calculateTax(calculateTotal(), paymentMethod) - voucherDiscount).toFixed(2)}`
                    )}
                  </button>
                </div>
              </div>
        ...
        <div className="flex items-center justify-between p-6 border-t border-[#2a2a2a] bg-[#1a1a1a]">
          ...
          <div className="flex space-x-3">
            {/* Debug info for newly added items */}
            {orderItems.filter(item => item.isNewlyAdded).length > 0 && (
              <div className="px-3 py-2 bg-[#f6b100]/20 border border-[#f6b100]/50 rounded-lg text-[#f6b100] text-sm">
                {orderItems.filter(item => item.isNewlyAdded).length} new item(s) ready for kitchen receipt
              </div>
            )}
            
            {/* Print New Kitchen Receipts Button */}
            {(newKitchenReceipts.length > 0 || (orderItems.some(item => item.isNewlyAdded) && hasBeenUpdated)) && (
              <button
                onClick={() => setShowNewKitchenReceiptsModal(true)}
                disabled={order.orderType === "DINE" && !hasBeenUpdated}
                className={`px-6 py-2 rounded-lg transition-colors flex items-center space-x-2 ${
                  order.orderType === "DINE" && !hasBeenUpdated
                    ? 'bg-gray-400 text-gray-600 cursor-not-allowed opacity-50'
                    : 'bg-[#f59e0b] text-white hover:bg-[#d97706]'
                }`}
                title={order.orderType === "DINE" && !hasBeenUpdated ? "Please update the order first before printing kitchen receipts" : ""}
              >
                <FaUtensils />
                <span>Print New Kitchen Receipts ({newKitchenReceipts.length || orderItems.filter(item => item.isNewlyAdded).length})</span>
                {order.orderType === "DINE" && !hasBeenUpdated && <span className="text-xs ml-1">(Update First)</span>}
              </button>
            )}
            ...
            {orderItems.filter(item => item.isNewlyAdded).length > 0 && (
              <button
                onClick={() => {
                  console.log("Manual trigger - showNewKitchenReceiptsModal:", showNewKitchenReceiptsModal);
                  console.log("Manual trigger - newKitchenReceipts:", newKitchenReceipts);
                  setShowNewKitchenReceiptsModal(true);
                }}
                disabled={order.orderType === "DINE" && !hasBeenUpdated}
              ...
              >
                ...
                {order.orderType === "DINE" && !hasBeenUpdated && " (Update Order First)"}
              </button>
            )}
            
            {order.orderStatus === "COMPLETED" && (
              <>
                {/* Different buttons for DELIVERY/PICKUP vs DINE orders */}
                {(order.orderType === "DELIVERY" || order.orderType === "PICKUP") ? (
                  <>
                    {/* For DELIVERY/PICKUP: Only show Print Customer Receipts button */}
                    <button
                      onClick={() => {
                        generateCustomerReceiptsMutation.mutate({
                          orderId: order._id
                      ...
                      disabled={isPrintingCustomerReceipts || isCompletingOrder}
                      className={`px-6 py-2 rounded-lg transition-all duration-200 ${
                        isPrintingCustomerReceipts || isCompletingOrder
                          ? 'bg-gray-400 text-gray-600 cursor-not-allowed'
                          : 'bg-[#8b5cf6] text-white hover:bg-[#7c3aed]'
                      }`}
                    >
                  ...
                  <>
                    {/* For DINE orders: Show all original buttons */}
                    <button
                      onClick={() => {
                        generateCustomerReceiptsMutation.mutate({
                          orderId: order._id
                    ...
                    {order.paymentStatus === "UNPAID" && (
                      <button
                        onClick={handleChangeToPaid}
                        disabled={updateOrderStatusMutation.isLoading || isAnyOperationInProgress}
                        className={`px-6 py-2 rounded-lg transition-all duration-200 ${
                          updateOrderStatusMutation.isLoading || isAnyOperationInProgress
                            ? 'bg-gray-400 text-gray-600 cursor-not-allowed'
            ...
            {order.orderStatus === "IN_PROGRESS" && (
              <>
                {/* Special buttons for DELIVERY/PICKUP orders */}
                {(order.orderType === "DELIVERY" || order.orderType === "PICKUP") ? (
                  <>
                    <button
                      onClick={handlePrintUnpaidReceipt}
                      disabled={isPrintingUnpaidReceipt || isPrintingCustomerReceipts || isCompletingOrder}
                      className={`px-6 py-2 rounded-lg transition-all duration-200 ${
                        isPrintingUnpaidReceipt || isPrintingCustomerReceipts || isCompletingOrder
                          ? 'bg-gray-400 text-gray-600 cursor-not-allowed'
                          : 'bg-[#8b5cf6] text-white hover:bg-[#7c3aed]'
                      }`}
                    >
                      {isPrintingUnpaidReceipt || isPrintingCustomerReceipts ? (
                        <div className="flex items-center space-x-2">
                          <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
                          <span>Printing...</span>
                        </div>
                    ...
                    <button
                      onClick={handleCompleteDeliveryPickup}
                      disabled={isCompletingOrder || isPrintingUnpaidReceipt || isPrintingCustomerReceipts}
                      className={`px-6 py-2 rounded-lg transition-all duration-200 ${
                        isCompletingOrder || isPrintingUnpaidReceipt || isPrintingCustomerReceipts
                          ? 'bg-gray-400 text-gray-600 cursor-not-allowed'
                          : 'bg-[#22c55e] text-white hover:bg-[#16a34a]'
                      }`}
                    >
                  ...
                  <>
                    ...
                    <button
                      ...
                      disabled={isUpdatingOrder || isCompletingOrder || (orderItems.length === 0 && (!safeOrder.deals || safeOrder.deals.length === 0))}
                      className={`px-6 py-2 rounded-lg transition-all duration-200 ${
                        isUpdatingOrder || isCompletingOrder || (orderItems.length === 0 && (!safeOrder.deals || safeOrder.deals.length === 0))
                          ? 'bg-gray-400 text-gray-600 cursor-not-allowed'
                    ...
                    >
                      {isUpdatingOrder ? (
                        <div className="flex items-center space-x-2">
                          <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-[#1f1f1f]"></div>
                          <span>Updating...</span>
                        </div>
                      ) : (
                        "Update Order"
          ...
          {updatedOrderData && updatedOrderData.pricingDetails && (
            <div className="mt-6 p-4 bg-[#10b981]/20 border border-[#10b981]/50 rounded-lg">
              <h4 className="text-[#10b981] font-bold mb-2">✓ Order Completed Successfully!</h4>
              <div className="space-y-1 text-sm">
                <div className="flex justify-between">
                  <span className="text-[#a0a0a0]">Final Total:</span>
                  <span className="text-[#10b981] font-bold">Rs{updatedOrderData.pricingDetails.finalTotal}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#a0a0a0]">Total Savings:</span>
                  <span className="text-[#10b981]">Rs{updatedOrderData.pricingDetails.totalSavings}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#a0a0a0]">Payment Method:</span>
                  <span className="text-[#f5f5f5]">{updatedOrderData.paymentMethod}</span>
                </div>
              </div>
            </div>
          )}
      ...
      {showPrintReceiptsModal && (
        console.log("Opening print receipts modal with data:", receiptData),
      ...
      {showNewKitchenReceiptsModal && (
        ...
        orderData={{
          ...
          customerReceipts={[]}
          kitchenReceipts={newKitchenReceipts.filter(receipt => !receipt.isDealReceipt)}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[60]">
          <div className="bg-[#1a1a1a] rounded-lg max-w-md w-full mx-4 p-6 max-h-[80vh] overflow-y-auto">
            ...
            <div className="mb-4 p-3 bg-[#262626] rounded-lg">
              <div className="text-[#f5f5f5] font-medium">{selectedItem.name}</div>
              <div className="text-[#a0a0a0] text-sm">Base Price: Rs{selectedItem.price}</div>
            </div>

            <div className="space-y-3 mb-6">
              <h4 className="text-[#f5f5f5] font-medium">Available Options:</h4>
              {selectedItem.options && selectedItem.options.length > 0 ? (
                selectedItem.options.map((option, index) => (
                  ...
                  >
                    ...
                    <span className="text-[#f6b100] font-medium">+Rs{option.price || 0}</span>
                  </label>
                ))
              ) : (
            ...
            {selectedOptions.length > 0 && (
              <div className="mb-4 p-3 bg-[#2a2a2a] rounded-lg">
                <div className="text-[#f5f5f5] font-medium mb-2">Selected Options:</div>
                {selectedOptions.map((option, index) => (
                  <div key={index} className="flex justify-between text-sm">
                    <span className="text-[#a0a0a0]">{option.name}</span>
                    <span className="text-[#f6b100]">+Rs{option.price || 0}</span>
                  </div>
                ))}
                <div className="border-t border-[#404040] mt-2 pt-2 flex justify-between font-medium">
                  ...
                  <span className="text-[#f6b100]">
                    Rs{selectedItem.price + selectedOptions.reduce((sum, opt) => sum + (opt.price || 0), 0)}
                  </span>
                </div>
              </div>
            )}
      ...
      {showPaymentModal && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[60]">
          <div className="bg-[#1a1a1a] rounded-lg max-w-md w-full mx-4 p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-[#f5f5f5]">Select Payment Method</h3>
              <button
              ...
              {["CASH", "CARD", "ONLINE"].map((method) => (
                ...
                >
                  <input
                    ...
                    className="text-[#f6b100] focus:ring-[#f6b100]"
                  />
                  <span className="text-[#f5f5f5] font-medium">{method}</span>
                </label>
              ))}
            </div>
            ...
            <div className="flex justify-end space-x-3">
              ...
              <button
                ...
                className="px-4 py-2 bg-[#10b981] text-white rounded-lg hover:bg-[#059669] transition-colors disabled:opacity-50"
              >
                {paymentMutation.isLoading ? "Processing..." : "Generate Receipt"}
              </button>
            </div>
          </div>
        </div>
      ...
      {showStatusModal && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[60]">
          <div className="bg-[#1a1a1a] rounded-lg max-w-md w-full mx-4 p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-[#f5f5f5]">Change Payment Status</h3>
              <button
            ...
            <div className="mb-6">
              <div className="text-[#f5f5f5] mb-4">
                Current Status: <span className={`font-bold ${
                  order.paymentStatus === 'PAID' ? 'text-[#10b981]' : 'text-[#ef4444]'
              ...
              <div className="space-y-3">
                ...
                <label className="flex items-center space-x-3 p-3 bg-[#262626] rounded-lg hover:bg-[#404040] transition-colors cursor-pointer">
                  ...
                  <div className="flex items-center space-x-2">
                    <span className="text-lg">❌</span>
                    <span className="text-[#f5f5f5] font-medium">UNPAID</span>
                  </div>
                </label>
              </div>
            </div>
            ...
            <div className="flex justify-end space-x-3">
              ...
              <button
                ...
                disabled={updateOrderStatusMutation.isLoading || newPaymentStatus === order.paymentStatus}
