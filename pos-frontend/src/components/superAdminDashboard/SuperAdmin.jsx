...
import { FaUserPlus, FaUsers, FaUtensils, FaEdit, FaTrash, FaEye, FaSignOutAlt, FaExclamationTriangle } from "react-icons/fa";
import { MdRestaurantMenu, MdAssignment } from "react-icons/md";
import { logout as logoutAPI, superAdminLogout, getAllAdmin, getCategoriesForAdmin, getAllItemsOfAdmin, getAllMenuOfAdmin, deleteCategory, updateCategory, deleteItem, updateItem, deleteMenu, updateMenu, getReceiptsByAdmin, getOrdersByAdmin, exportOrdersToExcel, getAllVouchers, getVouchersByAdmin, deleteVoucher, getDealsByAdmin, deleteDeal } from "../../https/index";
...
const setupSteps = [
  { id: 1, title: "Create User", description: "First, create a user account", completed: false },
  { id: 2, title: "Create Categories", description: "Create multiple categories for menu items", completed: false },
  { id: 3, title: "Create Items", description: "Add multiple menu items to categories", completed: false },
  { id: 4, title: "Create Menu", description: "Create the final menu with logo and items", completed: false },
];

// Mock data for demonstration
const mockUsers = [
  { id: 1, email: "user1@restaurant.com", role: "Manager", status: "Active", createdAt: "2025-01-15" },
  { id: 2, email: "user2@restaurant.com", role: "Cashier", status: "Active", createdAt: "2025-01-18" },
  { id: 3, email: "user3@restaurant.com", role: "Waiter", status: "Inactive", createdAt: "2025-01-20" },
];
...
const mockAdmins = [
  ...
  { 
    ...
    totalSales: "Rs 18,950",
    ordersCount: 567
  },
];

const mockCategories = [
  { id: 1, name: "Appetizers", itemCount: 8, status: "Active" },
  { id: 2, name: "Main Course", itemCount: 15, status: "Active" },
  { id: 3, name: "Desserts", itemCount: 6, status: "Active" },
  { id: 4, name: "Beverages", itemCount: 12, status: "Active" },
  { id: 5, name: "Breakfast", itemCount: 10, status: "Inactive" },
];

const mockRestaurants = [
  { id: 1, name: "Spice Garden Restaurant", cuisine: "Indian", location: "Downtown", status: "Active", createdAt: "2025-01-10" },
  { id: 2, name: "Pizza Palace", cuisine: "Italian", location: "Mall Street", status: "Active", createdAt: "2025-01-12" },
  { id: 3, name: "Burger Hub", cuisine: "American", location: "Food Court", status: "Inactive", createdAt: "2025-01-14" },
];

const mockMenuItems = [
  { id: 1, name: "Butter Chicken", category: "Main Course", price: "Rs 15.99", restaurant: "Spice Garden Restaurant", status: "Available" },
  { id: 2, name: "Masala Dosa", category: "Breakfast", price: "Rs 8.99", restaurant: "Spice Garden Restaurant", status: "Available" },
  { id: 3, name: "Margherita Pizza", category: "Main Course", price: "Rs 12.99", restaurant: "Pizza Palace", status: "Available" },
  { id: 4, name: "Classic Burger", category: "Fast Food", price: "Rs 9.99", restaurant: "Burger Hub", status: "Unavailable" },
];
...
const SuperAdmin = () => {
  ...
  const calculateAnalytics = (orders) => {
      ...
      try {
        ...
        if (order.createdAt) {
          ...
          if (!isNaN(orderDate.getTime())) {
            ...
            const dayIndex = last30Days.findIndex(day => day.date === orderDateString);
            if (dayIndex !== -1) {
              let orderTotal = 0;
              if (order.items && order.items.length > 0) {
                order.items.forEach(item => {
        ...
        console.warn('Invalid date found in order:', order._id, order.createdAt);
      }
    });
    ...
    console.log('Deal analytics:', { totalDealRevenue, totalDealsSold, totalDealsSavings, topDeals });
    ...
    return {
      ...
      dealMetrics: {
        ...
        totalDealsSavings,
        topDeals,
        dealPercentageOfSales: totalSales > 0 ? (totalDealRevenue / totalSales) * 100 : 0
      }
    };
  };
  ...
  const handleExportToExcel = async (adminId) => {
    try {
      ...
      if (response.data?.success) {
        ...
        // Generate filename
        const filename = `orders_${selectedAdmin.email}_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  const handleExportSingleOrder = async (order) => {
    try {
      ...
      const summaryData = [
        ...
        ['Generated on:', new Date().toLocaleDateString('en-GB'), new Date().toLocaleTimeString('en-US'), ''],
        ['', '', '', ''],
        ['📋 ORDER INFORMATION', '', '', ''],
        ...
        ['Gross Total:', `Rs ${(totalBasePrice + totalOptionsPrice).toFixed(2)}`, '', ''],
        ...
        ['Total Savings:', `Rs ${(totalItemDiscount + voucherDiscount).toFixed(2)}`, '', '']
      ];
      ...
      const itemsData = [
        ['🛍️ ITEMS BREAKDOWN', '', '', '', '', '', '', '', '', '', '', ''],
        ['', '', '', '', '', '', '', '', '', '', '', ''],
        ['#', 'Item Name', 'Category', 'Menu', 'Qty', 'Base Price', 'Options', 'Options Price', 'Discount', 'Tax Rate', 'Tax Amount', 'Subtotal']
      ];
      ...
      const calculationData = [
        ...
        ['3', 'Gross Amount', `Rs ${(totalBasePrice + totalOptionsPrice).toFixed(2)}`],
        ['4', 'Less: Item Discounts', `Rs ${totalItemDiscount.toFixed(2)}`],
        ['5', 'Amount After Discounts', `Rs ${(totalBasePrice + totalOptionsPrice - totalItemDiscount).toFixed(2)}`],
      ...
      XLSX.utils.book_append_sheet(workbook, calculationWorksheet, 'Calculations');
      
      // Generate filename with timestamp
      const orderId = order._id ? String(order._id).slice(-8).toUpperCase() : 'ORDER';
  ...
  }, [selectedAdmin, fetchCategoriesForAdmin, fetchItemsForAdmin, fetchMenuForAdmin, fetchOrdersAndAnalytics]);
  ...
  const handleUpdateCategory = useCallback(async (newName) => {
    ...
    try {
      const response = await updateCategory(selectedAdmin._id, editingCategory._id, { name: newName.trim() });
      if (response.data?.success) {
        // Refresh categories for the admin
        ...
        <div className="flex-1">
          {isEditing ? (
            ...
            <div>
              ...
              <p className="text-[#a0a0a0] text-sm">Created: {new Date(category.createdAt).toLocaleDateString()}</p>
            </div>
          )}
        </div>
        {!isEditing && (
  ...
  const handleUpdateMenu = useCallback(async (menuData, logoFile = null) => {
    ...
    try {
      ...
      console.log('About to call updateMenu with:', { adminId, menuId: menuData._id, requestData });
  ...
  const handleItemRemovedFromMenu = useCallback(async (removedItem, updatedMenu) => {
  ...
  }, [voucherToDelete, fetchAllVouchers, fetchVouchersForAdmin, selectedAdmin, adminDetailView]);
  ...
  const handleEditMenuClick = useCallback((menu) => {
  ...
  const handleItemAddedDuringMenuEdit = useCallback(async (stepId, data = null) => {
  ...
  const handleLogoutConfirm = async () => {
    ...
    try {
      ...
      if (error.response?.data) {
        ...
        console.error('Server error details:', { message, serverError, status: error.response.status });
        
        // Handle specific error cases
        if (error.response.status === 401) {
          console.warn('Token invalid or expired - user session already cleared');
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[#a0a0a0] text-sm leading-relaxed">
              Are you sure you want to logout? You will be redirected to the login page and will need to sign in again to access the dashboard.
            </p>
          </div>
        </div>
      ...
      <div className="bg-[#1a1a1a] rounded-2xl border border-[#404040] p-6 max-w-md w-full mx-4 shadow-2xl">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[#a0a0a0] text-sm leading-relaxed">
              Are you sure you want to delete the category "<span className="text-[#f5f5f5] font-medium">{categoryToDelete?.name}</span>"? This action cannot be undone and may affect associated menu items.
            </p>
          </div>
        </div>
        <div className="mt-6 flex gap-3">
          ...
          <button
            onClick={() => handleDeleteCategory(selectedAdmin._id, categoryToDelete._id)}
            disabled={isDeletingCategory}
            className={`flex-1 py-2 px-4 rounded-lg font-medium transition-colors ${
              isDeletingCategory
                ? 'bg-[#dc2626] opacity-50 cursor-not-allowed text-white'
      ...
      <div className="bg-[#1a1a1a] rounded-2xl border border-[#404040] p-6 max-w-md w-full mx-4 shadow-2xl">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[#a0a0a0] text-sm leading-relaxed">
              Are you sure you want to delete the item "<span className="text-[#f5f5f5] font-medium">{itemToDelete?.name}</span>"? This action cannot be undone and will remove the item from all menus.
            </p>
          </div>
        </div>
        <div className="mt-6 flex gap-3">
          ...
          <button
            onClick={() => handleDeleteItem(selectedAdmin._id, itemToDelete._id)}
            disabled={isDeletingItem}
            className={`flex-1 py-2 px-4 rounded-lg font-medium transition-colors ${
              isDeletingItem
                ? 'bg-[#dc2626] opacity-50 cursor-not-allowed text-white'
      ...
      <div className="bg-[#1a1a1a] rounded-2xl border border-[#404040] p-6 max-w-md w-full mx-4 shadow-2xl">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[#a0a0a0] text-sm leading-relaxed">
              Are you sure you want to delete the menu "<span className="text-[#f5f5f5] font-medium">{menuToDelete?.name}</span>"? This action cannot be undone and will remove the entire menu.
            </p>
          </div>
        </div>
        <div className="mt-6 flex gap-3">
  ...
  const SetupWizardTab = () => (
    <div className="container mx-auto px-6 md:px-4">
      <div className="bg-[#1a1a1a] rounded-lg p-6">
        <h3 className="text-xl font-bold text-[#f5f5f5] mb-6">Restaurant Setup Wizard</h3>
        <p className="text-[#a0a0a0] mb-8">Follow these steps to set up a complete restaurant system</p>
          ...
          <div className={`p-6 rounded-lg border-2 ${
            ...
            <div className="flex justify-between items-start mb-4">
              ...
              <div className="flex flex-col gap-2 ml-4">
                ...
                {currentStep === 2 && createdCategories.length > 0 && !completedSteps.includes(2) && (
                  <button
          ...
          <div className={`p-6 rounded-lg border-2 ${
            ...
            <div className="flex justify-between items-start mb-4">
              ...
              <div className="flex flex-col gap-2 ml-4">
                ...
                {currentStep === 3 && createdItems.length > 0 && !completedSteps.includes(3) && (
                  <button
          ...
          <div className={`p-6 rounded-lg border-2 ${
            ...
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <p className="text-[#a0a0a0] text-sm">
                  Create the final menu with logo, name and select items to include
                </p>
                {createdMenu && (
                  <div className="mt-3 p-3 bg-[#1a2e1a] rounded border border-green-600">
                    <p className="text-green-300 text-sm">
        ...
        {completedSteps.length === 4 && (
          <div className="mt-8 p-6 bg-gradient-to-r from-green-900 to-green-800 rounded-lg border border-green-600">
            <div className="text-center">
              ...
              <h3 className="text-xl font-bold text-green-100 mb-2">Setup Complete!</h3>
              <p className="text-green-200 mb-4">
                Your restaurant system is now ready with user, categories, items, and menu. The setup process has been completed successfully!
              </p>
              <button
  ...
  const AllAdminsTab = () => {
          ...
          <div className="bg-[#1a1a1a] rounded-lg p-6">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-bold text-[#f5f5f5]">{selectedAdmin.name}</h3>
                  <p className="text-[#a0a0a0]">{selectedAdmin.email} - Admin Dashboard</p>
                </div>
              </div>
            ...
            <div className="mb-6">
              <h2 className="text-2xl font-bold text-[#f5f5f5] mb-2">Admin Information</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div className="bg-[#262626] p-4 rounded-lg">
                  <h4 className="text-[#a0a0a0] text-sm">Total Categories</h4>
                  <p className="text-[#60a5fa] text-2xl font-bold">{adminCategories.length}</p>
                </div>
                <div className="bg-[#262626] p-4 rounded-lg">
                  <h4 className="text-[#a0a0a0] text-sm">Total Menu Items</h4>
                  <p className="text-[#10b981] text-2xl font-bold">{adminItems.length}</p>
                </div>
                <div className="bg-[#262626] p-4 rounded-lg">
            ...
            <div className="flex gap-3 mb-6">
              {["Analytics", "Categories", "Items", "Menu", "Vouchers", "Deals", "Settings"].map((view) => (
                <button
                  key={view}
                  onClick={() => {
            ...
            <div className="bg-[#262626] rounded-lg p-6">
                ...
                <div>
                  ...
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      {!isLoadingOrders && adminOrders && adminOrders.length > 0 && (
                        <button
                          onClick={() => handleExportToExcel(selectedAdmin._id)}
                          disabled={isExportingExcel}
                          className={`px-4 py-2 ${isExportingExcel ? 'bg-gray-500 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700'} text-white rounded-lg font-medium transition-colors flex items-center gap-2`}
                        >
                          {isExportingExcel ? (
                            <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    ...
                    <div className="bg-[#1a1a1a] rounded-lg p-6 border border-[#404040]">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-[#a0a0a0] text-sm font-medium">Total Sales</p>
                    ...
                    <div className="bg-[#1a1a1a] rounded-lg p-6 border border-[#404040]">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-[#a0a0a0] text-sm font-medium">Avg Order Value</p>
                          <p className="text-[#f5f5f5] text-2xl font-bold">
                    ...
                    <div className="bg-[#1a1a1a] rounded-lg p-6 border border-[#404040]">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-[#f5f5f5] text-2xl font-bold">
                            Rs{analyticsData.dealMetrics.totalDealRevenue.toFixed(2)}
                          </p>
                          <p className="text-[#f6b100] text-xs">
                            {analyticsData.dealMetrics.dealPercentageOfSales.toFixed(1)}% of total sales
                          </p>
                        </div>
                        <div className="p-3 bg-[#f6b100]/20 rounded-full">
                          <svg className="w-6 h-6 text-[#f6b100]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                      ...
                      <div className="bg-[#1a1a1a] rounded-lg p-6 border border-[#404040]">
                        <h4 className="text-[#f5f5f5] text-lg font-semibold mb-4 flex items-center">
                          <svg className="w-5 h-5 text-[#f6b100] mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
                          </svg>
                          Deal Performance
                        </h4>
                        <div className="space-y-4">
                          <div className="flex justify-between items-center">
                            <span className="text-[#a0a0a0]">Total Deals Sold</span>
                            <span className="text-[#f5f5f5] font-semibold">{analyticsData.dealMetrics.totalDealsSold}</span>
                          </div>
                          <div className="flex justify-between items-center">
                            <span className="text-[#a0a0a0]">Customer Savings</span>
                            <span className="text-green-400 font-semibold">Rs{analyticsData.dealMetrics.totalDealsSavings.toFixed(2)}</span>
                          </div>
                          <div className="flex justify-between items-center">
                            <span className="text-[#a0a0a0]">Revenue from Deals</span>
                            <span className="text-[#f6b100] font-semibold">Rs{analyticsData.dealMetrics.totalDealRevenue.toFixed(2)}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-[#1a1a1a] rounded-lg p-6 border border-[#404040]">
                        <h4 className="text-[#f5f5f5] text-lg font-semibold mb-4">Top Performing Deals</h4>
                        <div className="space-y-3">
                          {analyticsData.dealMetrics.topDeals.length > 0 ? (
                            analyticsData.dealMetrics.topDeals.map((deal, index) => (
                              <div key={deal.name} className="flex items-center justify-between p-3 bg-[#262626] rounded-lg">
                                <div className="flex items-center">
                                  <span className="w-6 h-6 bg-[#f6b100] text-[#1f1f1f] rounded-full flex items-center justify-center text-xs font-bold mr-3">
                                    {index + 1}
                                  </span>
                                  <div>
                                    <p className="text-[#f5f5f5] font-medium">{deal.name}</p>
                                    <p className="text-[#a0a0a0] text-sm">{deal.quantity} sold</p>
                                  </div>
                                </div>
                                <div className="text-right">
                                  <p className="text-[#f6b100] font-semibold">Rs{deal.totalRevenue.toFixed(2)}</p>
                                  <p className="text-green-400 text-sm">Save Rs{deal.totalSavings.toFixed(2)}</p>
                                </div>
                              </div>
                            ))
                          ) : (
                            <p className="text-[#a0a0a0] text-center py-4">No deal data available</p>
                          )}
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="mb-6">
                    <div className="relative">
                      ...
                      <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <svg className="h-5 w-5 text-[#a0a0a0]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  {isLoadingOrders ? (
                    ...
                    (() => {
                      ...
                      const filteredOrders = searchOrderId 
                        ? adminOrders.filter(order => {
                            const orderId = order._id ? String(order._id).slice(-8).toUpperCase() : `ORD-${adminOrders.indexOf(order) + 1}`;
                            return orderId.includes(searchOrderId.toUpperCase());
                          })
                        : adminOrders;

                      // Get orders to display (with pagination)
                      const ordersToDisplay = filteredOrders.slice(0, displayedOrdersCount);
                        ...
                        <div>
                          {filteredOrders.length === 0 ? (
                            <div className="text-center py-12 bg-[#1a1a1a] rounded-lg">
                              <div className="text-4xl mb-4">🔍</div>
                              <h4 className="text-[#f5f5f5] text-lg font-medium mb-2">No Orders Found</h4>
                              <p className="text-[#a0a0a0]">No orders match your search criteria.</p>
                              <p className="text-[#606060] text-sm mt-1">Try searching with a different Order ID.</p>
                            </div>
                          ) : (
                            <>
                              {searchOrderId && (
                                <div className="mb-4 p-3 bg-[#1a1a1a] border border-[#60a5fa] rounded-lg">
                                  <p className="text-[#60a5fa] text-sm">
                                    Found {filteredOrders.length} order{filteredOrders.length !== 1 ? 's' : ''} matching "{searchOrderId}"
                                    {displayedOrdersCount < filteredOrders.length && ` (showing first ${displayedOrdersCount})`}
                                  </p>
                                </div>
                              )}
                                ...
                                <table className="w-full text-left text-[#f5f5f5]">
                                  ...
                                  <tbody>
                                    {ordersToDisplay.map((order, index) => {
                                      ...
                                      const voucherDiscount = order.voucherDiscount || 0;
                                      const finalOrderTotal = totalFinalAmount + totalDealAmount - voucherDiscount;
                                      ...
                                      return (
                                        ...
                                        >
                                          <td className="p-4">
                                            #{order._id ? String(order._id).slice(-8).toUpperCase() : `ORD-${index + 1}`}
                                          </td>
                                          <td className="p-4">
                                            <span className={`px-2 py-1 rounded text-xs font-medium ${
                                              order.orderType === 'DINE' ? 'bg-blue-900 text-blue-300' :
                                              order.orderType === 'DELIVERY' ? 'bg-green-900 text-green-300' :
                                              order.orderType === 'PICKUP' ? 'bg-orange-900 text-orange-300' :
                                          ...
                                          <td className="p-4">
                                            <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                                              order.orderStatus === 'COMPLETED' || order.paymentStatus === 'PAID'
                                                ...
                                                : order.orderStatus === 'IN_PROGRESS' || order.paymentStatus === 'PENDING'
                                                ? 'bg-yellow-900 text-yellow-300'
                                                : 'bg-blue-900 text-blue-300'
                                            }`}>
                                              {order.orderStatus || order.paymentStatus || 'COMPLETED'}
                                            </span>
                                          </td>
                                          <td className="p-4">
                                            {(() => {
                                              if (!order.createdAt) return new Date().toLocaleDateString() + ' ' + new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                                              try {
                                                const date = new Date(order.createdAt);
                                                if (isNaN(date.getTime())) return new Date().toLocaleDateString() + ' ' + new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                              ...
                              {hasMoreOrders && (
                                <div className="text-center mt-6">
                                  <button
                                    onClick={() => setDisplayedOrdersCount(prev => prev + 5)}
                                    className="px-6 py-3 bg-[#60a5fa] hover:bg-[#3b82f6] text-white rounded-lg font-medium transition-colors"
                                  >
                                    See More Orders ({filteredOrders.length - displayedOrdersCount} remaining)
                                  </button>
                                </div>
                              )}

                              {/* Show All/Show Less Toggle */}
                              {filteredOrders.length > 5 && displayedOrdersCount >= filteredOrders.length && (
                    ...
                    <div className="text-center py-12 bg-[#1a1a1a] rounded-lg">
                      ...
                      <h4 className="text-[#f5f5f5] text-lg font-medium mb-2">No Orders Found</h4>
                      <p className="text-[#a0a0a0]">This restaurant hasn't received any orders yet.</p>
                      <p className="text-[#606060] text-sm mt-1">Orders will appear here once customers start placing them.</p>
                    </div>
                  )}
                        ...
                        <div className="mt-6">
                          {searchOrderId && (
                            <h4 className="text-[#f5f5f5] font-medium mb-3">
                              Statistics for "{searchOrderId}" ({statsOrders.length} orders)
                            </h4>
                          )}
                          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <div className="bg-[#1a1a1a] p-4 rounded-lg text-center">
                              ...
                              <p className="text-[#a0a0a0] text-sm">
                                {searchOrderId ? 'Matching Orders' : 'Total Orders'}
                              </p>
                            </div>
                            <div className="bg-[#1a1a1a] p-4 rounded-lg text-center">
                              <h4 className="text-[#10b981] text-2xl font-bold">
                                Rs{statsOrders.reduce((sum, order) => {
                                  let orderTotal = 0;
                                  if (order.items && order.items.length > 0) {
                                    order.items.forEach(item => {
                                      const basePrice = item.basePrice || item.originalPrice || item.price || 0;
                                  ...
                                  return sum + (orderTotal - (order.voucherDiscount || 0));
                                }, 0).toFixed(2)}
                              </h4>
                              <p className="text-[#a0a0a0] text-sm">Total Revenue</p>
                            </div>
                            <div className="bg-[#1a1a1a] p-4 rounded-lg text-center">
                              <h4 className="text-[#f59e0b] text-2xl font-bold">
                                Rs{statsOrders.length > 0 
                                  ? (statsOrders.reduce((sum, order) => {
                                      ...
                                      return sum + (orderTotal - (order.voucherDiscount || 0));
                                    }, 0) / statsOrders.length).toFixed(2)
                                  : '0.00'
                                }
                              </h4>
                              <p className="text-[#a0a0a0] text-sm">Average Order</p>
                            </div>
                            <div className="bg-[#1a1a1a] p-4 rounded-lg text-center">
                              <h4 className="text-[#8b5cf6] text-2xl font-bold">
                                {statsOrders.reduce((sum, order) => sum + (order.items?.length || 0), 0)}
                              </h4>
                              <p className="text-[#a0a0a0] text-sm">Total Items Sold</p>
                            </div>
                          </div>
                        </div>
                      ) : null;
              ...
              {adminDetailView === "Categories" && (
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-bold text-[#f5f5f5]">Categories ({adminCategories.length})</h3>
                    {isLoadingCategories && (
                  ...
                  {isLoadingCategories ? (
                    ...
                    <div className="text-center py-8">
                      <p className="text-[#a0a0a0]">No categories found for this admin</p>
                      <button 
                        onClick={() => setIsCreateCategoryModalOpen(true)}
              ...
              {adminDetailView === "Items" && (
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-bold text-[#f5f5f5]">Menu Items ({adminItems.length})</h3>
                    {isLoadingItems && (
                      <div className="text-[#60a5fa] text-sm">Loading...</div>
                    )}
                  </div>
                  ...
                  {isLoadingItems ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {adminItems.map(item => (
                        <div key={item._id} className="bg-[#1a1a1a] p-4 rounded-lg">
                          <div className="flex items-center gap-3 mb-3">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-[#a0a0a0] text-sm">Category: {item.categoryId?.name || 'N/A'}</p>
                            </div>
                          </div>
                          <p className="text-[#60a5fa] font-bold mb-3">Rs{item.price}</p>

                          {/* Display Options */}
                          {item.options && item.options.length > 0 && (
                            <div className="mb-3 p-2 bg-[#262626] rounded border border-[#404040]">
                              <h5 className="text-[#f5f5f5] text-xs font-medium mb-2">Options ({item.options.length})</h5>
                              <div className="space-y-1">
                                {item.options.map((option, optionIndex) => (
                                  <div key={optionIndex} className="flex justify-between items-center text-xs">
                                    <span className="text-[#a0a0a0]">{option.name}</span>
                                    <span className="text-[#10b981] font-medium">+Rs{option.price}</span>
                                  </div>
                                ))}
                              </div>
                            </div>
                    ...
                    <div className="text-center py-8">
                      <p className="text-[#a0a0a0]">No menu items found for this admin</p>
                      <button 
                        onClick={() => setIsCreateItemModalOpen(true)}
                        className="mt-4 px-4 py-2 bg-[#60a5fa] hover:bg-[#3b82f6] text-white rounded-lg"
                      >
              ...
              {adminDetailView === "Menu" && (
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-bold text-[#f5f5f5]">Restaurant Menu</h3>
                    {isLoadingMenu && (
                      <div className="text-[#60a5fa] text-sm">Loading menu...</div>
                    )}
                  </div>
                  ...
                  {isLoadingMenu ? (
                    ...
                    <div className="space-y-6">
                      ...
                      {Array.isArray(adminMenu) ? (
                        adminMenu.map((menu, menuIndex) => (
                          <div key={menu._id} className="bg-[#1a1a1a] p-6 rounded-lg border border-[#404040]">
                            {/* Menu Header */}
                            <div className="flex items-center gap-4 mb-4">
                              ...
                              <div className="flex-1">
                                <div className="flex items-center justify-between">
                                  <div>
                                    <h4 className="text-[#f5f5f5] font-bold text-xl">{menu.name}</h4>
                                    <p className="text-[#a0a0a0] text-sm">Created: {new Date(menu.createdAt).toLocaleDateString()}</p>
                                    <p className="text-[#60a5fa] text-sm">{menu.itemsID?.length || 0} items in this menu</p>
                                  </div>
                                  <div className="flex items-center gap-3">
                                    <span className="px-2 py-1 rounded text-xs bg-green-900 text-green-300">
                                      Menu #{menuIndex + 1}
                            ...
                            <div>
                              <h6 className="text-[#f5f5f5] font-medium mb-3">
                                Items in "{menu.name}" ({menu.itemsID?.length || 0})
                              </h6>
                              ...
                              {menu.itemsID && menu.itemsID.length > 0 ? (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                  {menu.itemsID.map(item => {
                                    ...
                                    const processedItem = menuItems.find(mi => mi._id === item._id) || item;
                                    return (
                                      <div key={item._id} className="bg-[#262626] p-3 rounded-lg border border-[#505050]">
                                        <div className="flex items-center gap-2 mb-2">
                                          ...
                                          />
                                          <div className="flex-1">
                                            <h6 className="text-[#f5f5f5] font-medium text-sm">{item.name}</h6>
                                            <p className="text-[#a0a0a0] text-xs">
                                              Category: {processedItem.categoryId?.name || 'N/A'}
                                            </p>
                                          </div>
                                        ...
                                        {item.options && item.options.length > 0 && (
                                          <div className="mt-2 p-2 bg-[#1a1a1a] rounded border border-[#404040]">
                                            <h6 className="text-[#f5f5f5] text-xs font-medium mb-1">Options ({item.options.length})</h6>
                                            <div className="space-y-1">
                                              {item.options.map((option, optionIndex) => (
                                                <div key={optionIndex} className="flex justify-between items-center text-xs">
                                                  <span className="text-[#a0a0a0]">{option.name}</span>
                                ...
                                <div className="text-center py-4 bg-[#262626] rounded-lg">
                                  <p className="text-[#a0a0a0] text-sm">No items in this menu</p>
                                </div>
                              )}
                            </div>
                          </div>
                        ...
                        <div className="bg-[#1a1a1a] p-6 rounded-lg">
                          <div className="flex items-center gap-4 mb-4">
                            ...
                            <div className="flex-1">
                              <div className="flex items-center justify-between">
                                <div>
                                  <h4 className="text-[#f5f5f5] font-bold text-xl">{adminMenu.name}</h4>
                                  <p className="text-[#a0a0a0] text-sm">Created: {new Date(adminMenu.createdAt).toLocaleDateString()}</p>
                                  <p className="text-[#60a5fa] text-sm">{menuItems.length} items in menu</p>
                                </div>
                                <div className="flex items-center gap-2">
                                  <button 
                                  ...
                                  <button 
                                    onClick={() => handleDeleteMenuClick(adminMenu)}
                                    className="p-2 bg-[#404040] hover:bg-[#505050] rounded text-[#ef4444] transition-colors"
                                    title="Delete Menu"
                                  >
                                    <FaTrash size={14} />
                      ...
                      <div className="bg-[#1a1a1a] p-4 rounded-lg">
                        ...
                        {isLoadingMenuItems ? (
                          <div className="text-center py-4">
                            ...
                            <p className="text-[#a0a0a0] mt-2 text-sm">Loading menu items...</p>
                          </div>
                        ) : menuItems.length > 0 ? (
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {menuItems.map(item => (
                              <div key={item._id} className="bg-[#262626] p-4 rounded-lg border border-[#505050]">
                                <div className="flex items-center gap-3 mb-3">
                                  <img 
                                    src={item.pictureURL || 'https://img.freepik.com/premium-psd/beautiful-food-menu-design-template_1150977-218.jpg?w=360'} 
                                    alt={item.name}
                                  ...
                                  <div className="flex-1">
                                    <div className="flex justify-between items-start">
                                      <h6 className="text-[#f5f5f5] font-semibold">{item.name}</h6>
                                      <span className="px-2 py-1 rounded text-xs bg-green-900 text-green-300">
                                        In Menu
                                      </span>
                                    </div>
                                ...
                                <p className="text-[#60a5fa] font-bold mb-2">Rs{item.price}</p>
                                ...
                                {item.description && (
                                  <p className="text-[#a0a0a0] text-sm mb-3 line-clamp-2">{item.description}</p>
                                )}
                              </div>
                            ))}
                          </div>
                          ...
                          <div className="text-center py-4 bg-[#262626] rounded-lg">
                            ...
                            <p className="text-[#a0a0a0] text-sm">No items found across all menus</p>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="text-center py-8 bg-[#1a1a1a] rounded-lg">
                      ...
                      <p className="text-[#a0a0a0] mb-4">No menu created for this admin yet</p>
                      <button 
                        onClick={() => setIsCreateMenuModalOpen(true)}
              ...
              {adminDetailView === "Vouchers" && (
                <div>
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-bold text-[#f5f5f5]">Voucher Management</h3>
                    <button 
                  ...
                  {isLoadingVouchers ? (
                    ...
                    <div className="grid gap-4">
                      {allVouchers.map((voucher) => (
                        <div key={voucher._id} className="bg-[#1a1a1a] p-4 rounded-lg border border-[#404040]">
                          <div className="flex justify-between items-start">
                            <div className="flex-1">
                              <div className="flex items-center gap-3 mb-2">
                                <h4 className="text-[#f5f5f5] font-medium">Voucher Code: #{voucher.code}</h4>
                    ...
                    <div className="text-center py-8 bg-[#1a1a1a] rounded-lg">
                      <div className="text-[#404040] text-4xl mx-auto mb-3">🎟️</div>
                      <p className="text-[#a0a0a0] mb-4">No vouchers created yet</p>
                      <button 
                        onClick={handleCreateVoucher}
                        className="px-6 py-2 bg-[#60a5fa] hover:bg-[#3b82f6] text-white rounded-lg"
                      >
              ...
              {adminDetailView === "Deals" && (
                <div>
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-bold text-[#f5f5f5]">Deal Management</h3>
                    <button 
                  ...
                  {isLoadingDeals ? (
                    ...
                    <div className="grid gap-4">
                      {allDeals.map((deal) => (
                        <div key={deal._id} className="bg-[#1a1a1a] p-4 rounded-lg border border-[#404040]">
                          <div className="flex justify-between items-start">
                            <div className="flex-1">
                              <div className="flex items-center gap-3 mb-2">
                                <h4 className="text-[#f5f5f5] font-medium">{deal.name}</h4>
                                <span className="px-2 py-1 bg-green-600 text-white text-xs rounded">
                                  Rs{deal.dealPrice}
                                </span>
                                <span className={`px-2 py-1 text-xs rounded ${deal.isActive ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300'}`}>
                              ...
                              {deal.description && (
                                <p className="text-[#a0a0a0] text-sm mb-2">{deal.description}</p>
                              )}
                          ...
                          {deal.items && deal.items.length > 0 && (
                            <div className="mt-3 pt-3 border-t border-[#404040]">
                              <h5 className="text-[#f5f5f5] text-sm font-medium mb-2">Included Items:</h5>
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                {deal.items.map((item, index) => (
                                  <div key={index} className="text-xs text-[#a0a0a0] bg-[#262626] p-2 rounded">
                    ...
                    <div className="text-center py-8 bg-[#1a1a1a] rounded-lg">
                      <div className="text-[#404040] text-4xl mx-auto mb-3">🎯</div>
              ...
              {adminDetailView === "Settings" && (
                <div>
                  <h3 className="text-lg font-bold text-[#f5f5f5] mb-4">Admin Settings</h3>
                  <div className="space-y-4">
                    <div className="bg-[#1a1a1a] p-4 rounded-lg">
                      <div className="flex justify-between items-start mb-2">
                        <h4 className="text-[#f5f5f5] font-medium">Account Status</h4>
                        <span className="px-2 py-1 bg-green-900 text-green-300 rounded text-xs">Active</span>
                      </div>
                      <p className="text-[#a0a0a0] text-sm mb-2">Admin account is active and operational</p>
                    ...
                    <div className="bg-[#1a1a1a] p-4 rounded-lg">
                      <h4 className="text-[#f5f5f5] font-medium mb-2">Quick Actions</h4>
    ...
    return (
      <div className="container mx-auto px-6 md:px-4">
        <div className="bg-[#1a1a1a] rounded-lg p-6">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl font-bold text-[#f5f5f5]">All Admins ({allAdmins.length})</h3>
            <div className="flex items-center gap-3">
              {isLoadingAdmins && (
                <div className="text-[#60a5fa] text-sm">Loading...</div>
              )}
          ...
          {isLoadingAdmins ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {allAdmins.map(admin => (
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h4 className="text-[#f5f5f5] font-bold text-lg">{admin.name}</h4>
                      <p className="text-[#a0a0a0] text-sm">{admin.email}</p>
                      <p className="text-[#60a5fa] text-sm">{admin.role || 'Admin'}</p>
                    </div>
                    <span className="px-2 py-1 rounded text-xs bg-green-900 text-green-300">
                      Active
                    </span>
                  ...
                  <div className="space-y-2 mb-4">
                    <div className="flex justify-between">
                      ...
                      <span className="text-[#f5f5f5] text-sm">{admin.phone || 'N/A'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-[#a0a0a0] text-sm">Admin ID:</span>
                      <span className="text-[#f5f5f5] text-sm text-xs">{admin._id?.slice(-8) || 'N/A'}</span>
                    </div>
                    <div className="flex justify-between">
            ...
            <div className="text-center py-12">
              ...
              <p className="text-[#606060] text-sm mt-2">Create your first admin user in the Setup Wizard</p>
            </div>
          )}
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#0a0a0a] p-4 md:p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <CreateCategoryModal
          ...
          isSettings={selectedAdmin && (adminDetailView === "Settings" || adminDetailView === "Categories")}
        />
        ...
        <CreateItemModal
          ...
          isSetupWizard={activeTab === "Setup Wizard"}
          isSettings={selectedAdmin && (adminDetailView === "Settings" || adminDetailView === "Items")}
        ...
        <CreateMenuModal
          ...
          selectedAdmin={selectedAdmin}
          isSetupWizard={activeTab === "Setup Wizard"}
          isSettings={selectedAdmin && (adminDetailView === "Settings" || adminDetailView === "Menu")}
        />
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-[#1a1a1a] p-6 rounded-lg max-w-md w-full mx-4">
              ...
              <p className="text-[#a0a0a0] mb-6">
                Are you sure you want to delete voucher code #{voucherToDelete.code}? This action cannot be undone.
              </p>
        ...
        {showDeleteDealModal && dealToDelete && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-[#1a1a1a] p-6 rounded-lg max-w-md w-full mx-4">
              <div className="flex items-center gap-3 mb-4">
                <FaExclamationTriangle className="text-red-500 text-xl" />
                <h3 className="text-lg font-bold text-[#f5f5f5]">Delete Deal</h3>
              </div>
              <p className="text-[#a0a0a0] mb-6">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-blue-100 text-sm">
                      Order #{selectedOrderForDetails._id?.slice(-8).toUpperCase() || 'N/A'} - {selectedOrderForDetails.orderType || 'DINE'}
                    </p>
                  </div>
                  <div className="flex items-center gap-3">
                    {/* Export Button */}
                    <button
                      onClick={() => handleExportSingleOrder(selectedOrderForDetails)}
                  ...
                  return (
                    <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="bg-[#262626] p-4 rounded-lg">
                          <h3 className="text-[#f5f5f5] font-semibold mb-3">Order Information</h3>
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              <span className="text-[#a0a0a0]">Order ID:</span>
                              <span className="text-[#f5f5f5]">#{order._id?.slice(-8).toUpperCase() || 'N/A'}</span>
                            </div>
                            <div className="flex justify-between">
                            ...
                            <div className="flex justify-between">
                              <span className="text-[#a0a0a0]">Order Status:</span>
                              <span className={`px-2 py-1 rounded text-xs ${
                                order.orderStatus === 'COMPLETED' ? 'bg-green-900 text-green-300' :
                                order.orderStatus === 'IN_PROGRESS' ? 'bg-yellow-900 text-yellow-300' :
                                'bg-blue-900 text-blue-300'
                              }`}>
                            ...
                            <div className="flex justify-between">
                              ...
                              <span className={`px-2 py-1 rounded text-xs ${
                                order.paymentStatus === 'PAID' ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300'
                              }`}>
                            ...
                            <div className="flex justify-between">
                              ...
                              <span className="text-[#f5f5f5]">{order.paymentMethod || order.paymentType || 'CASH'}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-[#a0a0a0]">Date & Time:</span>
                        ...
                        <div className="bg-[#262626] p-4 rounded-lg">
                          ...
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              <span className="text-[#a0a0a0]">Customer Name:</span>
                              <span className="text-[#f5f5f5]">{order.customerInfo?.name || 'Walk-in Customer'}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-[#a0a0a0]">Phone:</span>
                              <span className="text-[#f5f5f5]">{order.customerInfo?.phone || 'N/A'}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-[#a0a0a0]">Address:</span>
                              <span className="text-[#f5f5f5] text-right max-w-[200px] break-words">{order.customerInfo?.address || 'N/A'}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-[#a0a0a0]">Table:</span>
                              <span className="text-[#f5f5f5]">{order.customerInfo?.table || order.tableNo || 'N/A'}</span>
                            </div>
                            {order.voucherCode && (
                              <div className="flex justify-between">
                                <span className="text-[#a0a0a0]">Voucher Code:</span>
                                <span className="text-[#10b981]">{order.voucherCode}</span>
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      {/* Items */}
                      <div className="bg-[#262626] p-4 rounded-lg">
                        <h3 className="text-[#f5f5f5] font-semibold mb-4">Order Items ({processedItems.length})</h3>
                        <div className="space-y-4">
                          {processedItems.map((item, index) => (
                            <div key={index} className="border-b border-[#404040] pb-4 last:border-b-0">
                              <div className="flex justify-between items-start mb-2">
                                <div className="flex-1">
                                  <h4 className="text-[#f5f5f5] font-medium">{item.name}</h4>
                                  {item.menuName && item.menuName !== 'General Items' && (
                                    <p className="text-[#a0a0a0] text-sm">{item.menuName}</p>
                                  )}
                                </div>
                                <div className="text-right">
                                  <div className="text-[#f5f5f5] font-semibold">x{item.quantity}</div>
                                  <div className="text-[#10b981] font-bold">Rs{(item.finalPrice * item.quantity).toFixed(2)}</div>
                                </div>
                              </div>
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                                <div>
                                  ...
                                  <div className="text-[#f5f5f5]">Rs{item.basePrice.toFixed(2)}</div>
                                </div>
                                {item.optionsPrice > 0 && (
                                  <div>
                                    <span className="text-[#a0a0a0]">Options:</span>
                                    <div className="text-[#f5f5f5]">Rs{item.optionsPrice.toFixed(2)}</div>
                                  </div>
                                )}
                                {item.itemDiscount > 0 && (
                                  <div>
                                    <span className="text-[#a0a0a0]">Discount:</span>
                                    <div className="text-[#10b981]">-Rs{item.itemDiscount.toFixed(2)}</div>
                                  </div>
                                )}
                                <div>
                                  <span className="text-[#a0a0a0]">Tax ({item.taxRate}%):</span>
                                  <div className="text-[#f59e0b]">Rs{item.taxAmount.toFixed(2)}</div>
                                </div>
                              </div>
                              
                              {item.options && item.options.length > 0 && (
                                <div className="mt-2">
                                  <span className="text-[#a0a0a0] text-xs">Options: </span>
                                  <span className="text-[#f5f5f5] text-xs">
                                    {item.options.map(opt => typeof opt === 'string' ? opt : `${opt.name} (+Rs${opt.price || 0})`).join(', ')}
                                  </span>
                                </div>
                              )}
                              ...
                              {item.notes && (
                                <div className="mt-2">
                                  ...
                                  <span className="text-[#f5f5f5] text-xs">{item.notes}</span>
                                </div>
                              )}
                            </div>
                          ))}
                      ...
                      {/* Order Summary */}
                      <div className="bg-[#262626] p-4 rounded-lg">
                        <h3 className="text-[#f5f5f5] font-semibold mb-4">Order Summary</h3>
                        <div className="space-y-2">
                          <div className="flex justify-between text-sm">
                            <span className="text-[#a0a0a0]">Original Total:</span>
                            <span className="text-[#f5f5f5]">Rs{totalOriginalAmount.toFixed(2)}</span>
                          </div>
                          {totalItemDiscount > 0 && (
                            <div className="flex justify-between text-sm">
                              <span className="text-[#a0a0a0]">Item Discounts:</span>
                              <span className="text-[#10b981]">-Rs{totalItemDiscount.toFixed(2)}</span>
                            </div>
                          )}
                          <div className="flex justify-between text-sm">
                            <span className="text-[#a0a0a0]">Subtotal:</span>
                            <span className="text-[#f5f5f5]">Rs{subtotal.toFixed(2)}</span>
                          </div>
                          {voucherDiscount > 0 && (
                            <div className="flex justify-between text-sm">
                              <span className="text-[#a0a0a0]">Voucher Discount:</span>
                              <span className="text-[#10b981]">-Rs{voucherDiscount.toFixed(2)}</span>
                            </div>
                          )}
                          {totalTaxAmount > 0 && (
                            <div className="flex justify-between text-sm">
                              <span className="text-[#a0a0a0]">Tax ({order.paymentMethod || order.paymentType || 'CASH'}):</span>
                              <span className="text-[#f59e0b]">+Rs{totalTaxAmount.toFixed(2)}</span>
                            </div>
                          )}
                          <div className="border-t border-[#404040] pt-2 mt-2">
                            <div className="flex justify-between font-bold text-lg">
                              <span className="text-[#f5f5f5]">FINAL TOTAL:</span>
                              <span className="text-[#10b981]">Rs{finalOrderTotal.toFixed(2)}</span>
                            </div>
                          </div>
                          {(totalItemDiscount + voucherDiscount) > 0 && (
                            <div className="flex justify-between text-sm">
                              <span className="text-[#a0a0a0]">Total Savings:</span>
                              <span className="text-[#10b981] font-semibold">Rs{(totalItemDiscount + voucherDiscount).toFixed(2)}</span>
                            </div>
                          )}
                        </div>
                      </div>
